...
3. For every mcq/multi_select/true_false question, write plausible distractor options and mark the correct one(s) yourself — never leave a question with no correct option marked.
4. For short_answer/fill_blank, options must be an empty array [] and the answer goes in correct_answer.
5. Assign difficulty ("easy"|"medium"|"hard") per question, matching what the teacher asked for if given.
6. Default points to 1 per question unless the teacher's description or instructions below specify otherwise.
...
${context?.studentContext || ""}`;
    ...
    if (ANTHROPIC_API_KEY) {
      ...
      const anthropicBody: any = {
        ...
        max_tokens: (action === "parse_questions" || action === "generate_questions") ? 8192 : 4096,
    ...
    if (action === "parse_questions" || action === "generate_questions") {
