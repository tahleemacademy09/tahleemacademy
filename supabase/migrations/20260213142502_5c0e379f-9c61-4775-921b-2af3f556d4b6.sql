...
CREATE POLICY "Admins/teachers can create exams" ON public.exams FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can update exams" ON public.exams FOR UPDATE USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can view all exams" ON public.exams FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
...
CREATE POLICY "Admins/teachers can update attempts for grading" ON public.exam_attempts FOR UPDATE USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can view all attempts" ON public.exam_attempts FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
...
CREATE POLICY "Admins/teachers can update answers for grading" ON public.exam_answers FOR UPDATE USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can view all answers" ON public.exam_answers FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Users can insert own answers" ON public.exam_answers FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid() AND ea.status = 'in_progress'));
CREATE POLICY "Users can update own answers during exam" ON public.exam_answers FOR UPDATE USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid() AND ea.status = 'in_progress'));
CREATE POLICY "Users can view own answers" ON public.exam_answers FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid()));
...
CREATE POLICY "Admins/teachers can create assignments" ON public.exam_assignments FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can delete assignments" ON public.exam_assignments FOR DELETE USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can manage assignments" ON public.exam_assignments FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Users can view own assignments" ON public.exam_assignments FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Admins can view all courses" ON public.courses FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
...
CREATE POLICY "Admins/teachers can delete violations" ON public.violations FOR DELETE USING (has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins/teachers can view all violations" ON public.violations FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Students can insert own violations" ON public.violations FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = violations.attempt_id AND ea.user_id = auth.uid()));
CREATE POLICY "Students can view own violations" ON public.violations FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = violations.attempt_id AND ea.user_id = auth.uid()));
...
CREATE POLICY "Admins/teachers can update proctoring sessions" ON public.proctoring_sessions FOR UPDATE USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Admins/teachers can view all proctoring sessions" ON public.proctoring_sessions FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Students can insert own proctoring session" ON public.proctoring_sessions FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));
CREATE POLICY "Students can update own proctoring session" ON public.proctoring_sessions FOR UPDATE USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));
CREATE POLICY "Students can view own proctoring session" ON public.proctoring_sessions FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));
...
CREATE POLICY "Admins can delete media" ON public.proctoring_media FOR DELETE USING (has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins/teachers can view all media" ON public.proctoring_media FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Students can insert own media" ON public.proctoring_media FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_media.attempt_id AND ea.user_id = auth.uid()));
CREATE POLICY "Students can view own media" ON public.proctoring_media FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_media.attempt_id AND ea.user_id = auth.uid()));
...
CREATE POLICY "Admins/teachers can view all device logs" ON public.device_logs FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Students can insert own device logs" ON public.device_logs FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = device_logs.attempt_id AND ea.user_id = auth.uid()));
...
CREATE POLICY "Admins can manage enrollments" ON public.enrollments FOR DELETE USING (has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can view all enrollments" ON public.enrollments FOR SELECT USING (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
CREATE POLICY "Users can enroll themselves" ON public.enrollments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own enrollment progress" ON public.enrollments FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Admins can create notifications" ON public.notifications FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'teacher'));
...
CREATE POLICY "Admins can create logs for any user" ON public.activity_logs FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));
