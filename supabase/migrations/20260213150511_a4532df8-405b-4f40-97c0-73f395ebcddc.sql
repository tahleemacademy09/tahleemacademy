...
-- Drop all RESTRICTIVE policies on exam_questions and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on exams and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on exam_attempts and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on exam_answers and recreate as PERMISSIVE
...
CREATE POLICY "Users can insert own answers" ON public.exam_answers
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid() AND ea.status = 'in_progress'));

CREATE POLICY "Users can update own answers during exam" ON public.exam_answers
  FOR UPDATE USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid() AND ea.status = 'in_progress'));

CREATE POLICY "Users can view own answers" ON public.exam_answers
  FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = exam_answers.attempt_id AND ea.user_id = auth.uid()));

-- Drop all RESTRICTIVE policies on exam_assignments and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on courses and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on violations and recreate as PERMISSIVE
...
CREATE POLICY "Students can insert own violations" ON public.violations
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = violations.attempt_id AND ea.user_id = auth.uid()));

CREATE POLICY "Students can view own violations" ON public.violations
  FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = violations.attempt_id AND ea.user_id = auth.uid()));

-- Drop all RESTRICTIVE policies on proctoring_sessions and recreate as PERMISSIVE
...
CREATE POLICY "Students can insert own proctoring session" ON public.proctoring_sessions
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));

CREATE POLICY "Students can update own proctoring session" ON public.proctoring_sessions
  FOR UPDATE USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));

CREATE POLICY "Students can view own proctoring session" ON public.proctoring_sessions
  FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_sessions.attempt_id AND ea.user_id = auth.uid()));

-- Drop all RESTRICTIVE policies on proctoring_media and recreate as PERMISSIVE
...
CREATE POLICY "Students can insert own media" ON public.proctoring_media
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_media.attempt_id AND ea.user_id = auth.uid()));

CREATE POLICY "Students can view own media" ON public.proctoring_media
  FOR SELECT USING (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = proctoring_media.attempt_id AND ea.user_id = auth.uid()));

-- Drop all RESTRICTIVE policies on device_logs and recreate as PERMISSIVE
...
CREATE POLICY "Students can insert own device logs" ON public.device_logs
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM exam_attempts ea WHERE ea.id = device_logs.attempt_id AND ea.user_id = auth.uid()));

-- Drop all RESTRICTIVE policies on enrollments and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on notifications and recreate as PERMISSIVE
...
-- Drop all RESTRICTIVE policies on activity_logs and recreate as PERMISSIVE
