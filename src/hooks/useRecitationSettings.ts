...
export interface RecitationSettings {
  ...
  surah_translation: string;   // English translation (optional display)
  // Instructions shown to student
  instructions:      string;
  tips:              string;   // comma-separated tips
  // Limits
  min_duration_sec:  number;   // minimum recording length
...
export const useRecitationSettings = () => {
  ...
  const save = async (updates: Partial<RecitationSettings>, adminId?: string) => {
