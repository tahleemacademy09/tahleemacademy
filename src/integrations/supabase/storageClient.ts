...
Fix: Use the MAIN supabase client.
...
export async function getSignedUrl(
  ...
  try {
    const { data, error } = await supabase.storage
      .from(bucket)      .createSignedUrl(fileUrl, expiresInSeconds);
    if (error) {
...
export async function testStorageConnection() {
  ...
  try {
    ...
    if (error) { console.error("❌ List buckets failed:", error.message); console.groupEnd(); return { success: false }; }
    
    const names = (buckets || []).map((b: any) => b.name);
    console.log("Buckets:", names.join(", ") || "(none)");
    
    if (!names.includes("subject-files")) console.warn("⚠️ Create 'subject-files' bucket in Supabase Dashboard → Storage");
    if (!names.includes("recordings"))    console.warn("⚠️ Create 'recordings' bucket in Supabase Dashboard → Storage");
    
    console.groupEnd();
    return { success: true, buckets: names };
  } catch (e) { console.error(e); console.groupEnd(); return { success: false }; }
}
