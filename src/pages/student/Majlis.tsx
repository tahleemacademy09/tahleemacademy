/*  src/pages/student/Majlis.tsx
    ENHANCED — True fullscreen overlay, working image & file uploads,
    all upload/display bugs fixed, better mobile UX
*/
...
import {
  ...
  Copy, Lock, Plus, Megaphone, Camera, Check,
...
// ── Helper: is this a storage path (not a URL or base64)? ─────────
...
const resolveMedia = async (path: string): Promise<string | null> => {
  if (!path) return null;
  if (!isStoragePath(path)) return path;                             // already a URL / base64
...
const AudioMsg = ({ path }: { path: string }) => {
  ...
  useEffect(() => {
    resolveMedia(path).then(u => { if (u) setUrl(u); else setErr(true); });
  }, [path]);
...
const ImageMsg = ({ path, text }: { path?: string | null; text?: string | null }) => {
  ...
  useEffect(() => {
    ...
    // text may carry a base64 fallback; path may be storage path or URL
    const src = path || text || "";
  ...
  if (loading) return (
    <div style={{ width: 180, height: 120, borderRadius: 8, background: "#e0e0e0", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Loader2 style={{ width: 20, height: 20, color: "#999", animation: "spin .8s linear infinite" }} />
    </div>
  );
  if (err || !url) return (
    <div style={{ width: 140, height: 80, borderRadius: 8, background: "#f0f0f0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4 }}>
      <Image style={{ width: 20, height: 20, color: "#bbb" }} />
      <span style={{ fontSize: 10, color: "#bbb" }}>Image unavailable</span>
    </div>
  );
  return (
    <img src={url} style={{ maxWidth: 220, maxHeight: 260, borderRadius: 8, cursor: "pointer", display: "block", objectFit: "cover" }}
      alt="shared image" loading="lazy" onClick={() => window.open(url, "_blank")} />
  );
};
...
const FileMsg = ({ path, text }: { path: string; text: string | null }) => {
  ...
  const open = async () => {
    setDownloading(true);
    const url = await resolveMedia(path);
    setDownloading(false);
    if (url) window.open(url, "_blank");
  };
  const filename = text || path.split("/").pop() || "file";
  ...
  return (
    <button onClick={open} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(0,0,0,0.06)", border: "none", cursor: "pointer", padding: "8px 12px", borderRadius: 10, minWidth: 160 }}>
      <div style={{ width: 36, height: 36, borderRadius: 8, background: WA_GREEN, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        {downloading ? <Loader2 style={{ width: 16, height: 16, color: "#fff", animation: "spin .8s linear infinite" }} /> : <FileText style={{ width: 16, height: 16, color: "#fff" }} />}
      </div>
      <div style={{ flex: 1, minWidth: 0, textAlign: "left" as const }}>
        <div style={{ fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const, maxWidth: 140 }}>{filename}</div>
...
const Majlis = ({ adminMode = false, onBroadcast, onCreateChannel }: MajlisProps) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      await supabase.from("chat_members" as any)
        ...
        .eq("channel_id", activeChannelId).eq("user_id", user!.id);
  ...
  const sendMessage = async (contentType = "text", mediaPath?: string, extraText?: string) => {
    ...
    const optimistic: any = {
      ...
      content_type: contentType, text, media_path: mediaPath || null,
      created_at: new Date().toISOString(), is_pinned: false,
      reply_to_id: replyTo?.id || null,
      reply_preview: replyTo ? (replyTo.text || "").slice(0, 100) : null,
    };
  ...
  const handleFileUpload = async (file: File, type: "image" | "file") => {
    ...
    try {
      ...
      if (!upErr) {
        // Success — send storage path
        await sendMessage(type, path, type === "file" ? file.name : null);
      } else if (type === "image") {
        // Fallback: convert to base64 and embed in text field
        const base64 = await new Promise<string>(res => {
          const r = new FileReader(); r.onloadend = () => res(r.result as string); r.readAsDataURL(file);
        });
        // Send as image with base64 in text
        const tempId = `temp-${Date.now()}`;
        const optimistic: any = {
          id: tempId, channel_id: activeChannelId, user_id: user.id,
          content_type: "image", text: base64, media_path: null,
          created_at: new Date().toISOString(), is_pinned: false,
        };
        setMessages(prev => [...prev, optimistic]);
        const { data: inserted, error: insErr } = await supabase.from("chat_messages").insert({
          ...
          user_id: user.id, content_type: "image", text: base64,
        }).select().single();
        if (insErr) {
          setMessages(prev => prev.filter(m => m.id !== tempId));
          ...
          setMessages(prev => prev.map(m => m.id === tempId ? inserted as unknown as ChatMessage : m));
          await supabase.from("chat_channels" as any).update({ last_message: "📷 Image", last_message_at: new Date().toISOString() }).eq("id", activeChannelId);
        }
      } else {
        toast({ title: "Upload failed — storage unavailable", description: "Run the storage setup SQL in Lovable.", variant: "destructive" });
      }
    } catch (e: any) {
  ...
  const handleProfileAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      if (!error) {
        ...
        // Fallback base64
        avatarUrl = await new Promise(res => { const r = new FileReader(); r.onloadend = () => res(r.result as string); r.readAsDataURL(file); });
      }
  ...
  const MsgBubble = ({ m, idx }: { m: ChatMessage; idx: number }) => {
    ...
    const nameColor  = userColors[name.charCodeAt(0) % userColors.length];

    return (
      <div style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start", marginBottom: isLast ? 8 : 2, padding: "0 10px" }}>
        {/* Avatar */}
        {!isMe && (
        ...
        <div style={{ display: "flex", flexDirection: "column", maxWidth: "74%", alignItems: isMe ? "flex-end" : "flex-start" }}>
          <div
            ...
            style={{
              ...
              borderRadius: isMe
                ? isFirst && isLast ? "12px 12px 2px 12px" : isLast ? "12px 12px 2px 12px" : "12px"
                : isFirst && isLast ? "12px 12px 12px 2px" : isLast ? "12px 12px 12px 2px" : "12px",
              boxShadow: "0 1px 1px rgba(0,0,0,.1)", outline: hl ? "2px solid #f6d860" : "none",
              marginTop: isFirst ? 4 : 1, opacity: isTemp ? 0.7 : 1,
            }}>
            {!isMe && isFirst && <div style={{ fontSize: 11, fontWeight: 700, color: nameColor, marginBottom: 2 }}>{name}</div>}
            {mIsPinned && (
            ...
            <div style={{ fontSize: 14, color: "#111", lineHeight: 1.45, wordBreak: "break-word" }} dir="auto">
              {m.content_type === "audio" && m.media_path
                ? <AudioMsg path={m.media_path} />
                : m.content_type === "image"
                ? <ImageMsg path={m.media_path} text={m.text} />
                : m.content_type === "file" && m.media_path
                ? <FileMsg path={m.media_path} text={m.text} />
          ...
          {showMessageMenu === `menu-${m.id}` && (
            <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 4px 24px rgba(0,0,0,.15)", minWidth: 180, overflow: "hidden", zIndex: 40, marginTop: 4 }}
              onClick={e => e.stopPropagation()}>
              {[
                ...
                ...(canModerate ? [{ icon: Pin, label: mIsPinned ? "Unpin" : "Pin", action: () => pinMessage(m), color: "#2196F3" }] : []),
  ...
  const DeleteSheet = ({ msgId }: { msgId: string }) => {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
        ...
        <div style={{ position: "relative", background: "#fff", borderRadius: "20px 20px 0 0", padding: "12px 0 32px", zIndex: 201 }}>
          ...
          {(m?.user_id === user?.id || canModerate) && (
            <button onClick={() => deleteForAll(msgId)}
              ...
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#FEE2E2", display: "flex", alignItems: "center", justifyContent: "center" }}><Trash2 style={{ width: 18, height: 18, color: "#EF4444" }} /></div>
              Delete for everyone
            </button>
          )}
          <button onClick={() => deleteForMe(msgId)}
            ...
            <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#f0f4f0", display: "flex", alignItems: "center", justifyContent: "center" }}><Trash2 style={{ width: 18, height: 18, color: "#666" }} /></div>
            Delete for me
          </button>
          <button onClick={() => setShowDeleteSheet(null)}
            style={{ width: "100%", padding: "16px 20px", background: "none", border: "none", cursor: "pointer", textAlign: "left" as const, fontSize: 16, color: WA_GREEN, fontWeight: 700, display: "flex", alignItems: "center", gap: 14, fontFamily: "'Cairo',sans-serif", borderTop: "1px solid #f0f0f0" }}>
            <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#e8f5e9", display: "flex", alignItems: "center", justifyContent: "center" }}><X style={{ width: 18, height: 18, color: WA_GREEN }} /></div>
            Cancel
          </button>
        </div>
      </div>
  ...
  const ProfileEditSheet = () => (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: "20px 20px 0 0", padding: "20px 20px 40px", zIndex: 201 }}>
  ...
  // ── RENDER ───────────────────────────────────────────────────
  return (
    // FULLSCREEN overlay — covers dashboard sidebar + header
    <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", fontFamily: "'Cairo',sans-serif", background: "#fff" }}
      ...
      <div style={{
        ...
        {/* Only render contents when sidebar is visible */}
        {!mobileShowChat && (
          <MajlisSidebar
            channels={channels} activeChannelId={activeChannelId}
        ...
        {activeChannel && !editingChannel && (
          <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: WA_GREEN, flexShrink: 0, zIndex: 10 }}>
              <button onClick={() => setMobileShowChat(false)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", padding: "4px 2px", marginRight: 2 }}>
              ...
              <div style={{ position: "relative" }}>
                <button onClick={e => { e.stopPropagation(); setShowHeaderMenu(v => !v); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.8)", cursor: "pointer", padding: 6 }}>
                  <MoreVertical style={{ width: 18, height: 18 }} />
                </button>
                {showHeaderMenu && (
                  <div style={{ position: "absolute", top: "100%", right: 0, background: "#fff", borderRadius: 8, boxShadow: "0 4px 20px rgba(0,0,0,.2)", minWidth: 200, zIndex: 50, overflow: "hidden" }}
                    onClick={e => e.stopPropagation()}>
                    {[
            ...
            {/* Upload progress */}
            {uploading && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", background: "#e8f5e9", borderBottom: "1px solid #c8e6c9", flexShrink: 0 }}>
                <Loader2 style={{ width: 14, height: 14, color: WA_GREEN, animation: "spin .8s linear infinite" }} />
                <span style={{ fontSize: 12, color: WA_GREEN, fontWeight: 600 }}>Uploading…</span>
              </div>
            )}
            ...
            {replyTo && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: "#f0f2f5", borderTop: "1px solid #e0e0e0", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#666", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{replyTo.text || "📎 Media"}</div>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 6, padding: "6px 8px", background: "#f0f2f5", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", background: "#fff", borderRadius: 24, flex: 1, padding: "0 12px", boxShadow: "0 1px 2px rgba(0,0,0,.1)", minWidth: 0 }}>
                  ...
                  <button onClick={() => imageInputRef.current?.click()} title="Send image"
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#999", padding: "0 4px", flexShrink: 0 }}>
                ...
                <button
                  ...
                  disabled={uploading}
                  style={{ width: 44, height: 44, borderRadius: "50%", background: uploading ? "#9ca3af" : isRecording ? "#EF4444" : WA_GREEN, border: "none", color: "#fff", cursor: uploading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 2px 6px rgba(0,0,0,.25)" }}>
