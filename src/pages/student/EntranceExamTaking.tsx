/**
 * EntranceExamTaking — Fixed Version with Proper Question Display
 */
...
const PreExamInstructions = ({
  ...
  return (    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: isMobile ? 8 : 10, marginBottom: isMobile ? 16 : 24 }}>
          {[
            { icon: "⏱️", v: `${exam?.time_limit_minutes || 15} min` },
            { icon: "❓", v: `${exam?.question_count || "?"} Questions` },
        ...
        {procEnabled && (
          <div style={{ background: "rgba(239,68,68,.1)", borderRadius: isMobile ? 10 : 12, padding: isMobile ? "12px" : "14px 16px", marginBottom: isMobile ? 16 : 20, border: "1px solid rgba(239,68,68,.25)", display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div>              <p style={{ fontWeight: 700, fontSize: isMobile ? 12 : 13, color: "#ef4444", margin: "0 0 4px" }}>🔒 Proctored Exam</p>
              <p style={{ fontSize: isMobile ? 11 : 12, color: "rgba(239,68,68,.8)", margin: 0, lineHeight: 1.5 }}>
                Your camera and activity will be monitored.
              </p>
            </div>
          </div>
        )}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ background: "rgba(239,68,68,.1)", borderRadius: isMobile ? 12 : 14, padding: isMobile ? "12px 16px" : "16px 20px", marginBottom: isMobile ? 16 : 24, border: "1px solid rgba(239,68,68,.25)" }}>
          <p style={{ fontSize: isMobile ? 12 : 14, fontWeight: 800, color: "#ef4444", margin: "0 0 8px", lineHeight: 1.3 }}>            {remaining === 1 ? "⚡ FINAL WARNING — Next violation will auto-submit!" : `You have ${remaining} violation${remaining !== 1 ? "s" : ""} remaining`}
          </p>
        </div>
...
const EntranceExamTaking = () => {
  ...
  // ── Load Exam Data ──────────────────────────────────────────────
  useEffect(() => {    if (!attemptId || !user) return;
    (async () => {
      try {
        ...
        if (att.status !== "in_progress") {
          // After completion, redirect to continue registration
          navigate("/onboarding?exam_completed=true"); 
          return;
        }
        ...
        console.log("Loaded questions:", qs); // Debug log
        setQuestions(qs || []);
        ...
        setProcConfig({
          ...
          fullscreen_required: ex.fullscreen_required ?? false,          tab_switch_limit: ex.tab_switch_limit ?? 3,
        ...
        toast({ title: "Error loading exam", variant: "destructive" });
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    const handleViolation = () => {
      ...
      if (count >= 3) {
        toast({
          ...
          description: "You left the exam page 3 times.",
          variant: "destructive",
    ...
    const handleBlur = () => {
      ...
      if (count >= 3) {
        toast({ title: "⚠️ Exam auto-submitted", variant: "destructive" });
        setTimeout(() => { if (!submittedRef.current) handleSubmit(); }, 1000);
      } else {
        setShowViolationWarning(true);
      }
  ...
  }, [showInstructions, loading, toast]);
  ...
  const saveAnswer = useCallback(async (questionId: string, answerText: string) => {
    ...
    if (ex) {
      await supabase.from("exam_answers")        .update({ answer_text: answerText, updated_at: new Date().toISOString() })
        .eq("id", ex.id);
    } else {
      await supabase.from("exam_answers")
        .insert({ attempt_id: attemptId!, question_id: questionId, answer_text: answerText });
    }
  }, [attemptId]);
  ...
  const toggleFlag = useCallback(async (questionId: string) => {
    ...
    setFlagged(prev => { 
      ...
      isNowFlagged ? n.add(questionId) : n.delete(questionId); 
      return n; 
    });
    ...
    try {
      ...
      for (const [qId, ans] of Object.entries(cur)) {
        if (ans) {
          ...
          if (ex) {
            await supabase.from("exam_answers").update({ answer_text: ans }).eq("id", ex.id);
          } else {
            await supabase.from("exam_answers")              .insert({ attempt_id: attemptId!, question_id: qId, answer_text: ans });
          }
        }
      }
      ...
      // Redirect to continue registration instead of results
      navigate("/onboarding?exam_completed=true", { replace: true });
  ...
  }, [submitting, attemptId, navigate, toast]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0b1f14", fontFamily: "'Cairo',sans-serif", paddingBottom: isMobile ? 80 : 16 }}>
      ...
      {showViolationWarning && (
        <ViolationWarning count={violationCount} onReturn={() => setShowViolationWarning(false)} />
      )}
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          padding: isMobile ? "6px 12px" : "8px 16px", borderRadius: 10, fontWeight: 900, 
          ...
          animation: isCritical ? "timerPulse 1s infinite" : "none",          flexShrink: 0,
        }}>
      ...
      <div style={{ 
        ...
        padding: isMobile ? "12px" : "16px", gap: isMobile ? "12px" : "16px",
        ...
        <div style={{ flex: isMobile ? "none" : 1, minWidth: 0, width: "100%" }}>
          ...
          {q ? (
              ...
              <div style={{ background: "rgba(255,255,255,.04)", padding: isMobile ? "12px 16px" : "14px 20px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid rgba(255,255,255,.06)", flexWrap: isMobile ? "wrap" : "nowrap" }}>
                ...
                <button onClick={() => toggleFlag(q.id)}
                  style={{ 
                    ...
                    cursor: "pointer", display: "flex", alignItems: "center", gap: isMobile ? 4 : 5,                     fontSize: isMobile ? 10 : 11, fontWeight: 700, 
                    color: flagged.has(q.id) ? "#f59e0b" : "rgba(255,255,255,.5)",
                    flexShrink: 0,
                  }}>
                  <Flag size={isMobile ? 10 : 12} /> {flagged.has(q.id) ? "Flagged" : "Flag"}
                </button>
              </div>
              ...
              <div style={{ padding: isMobile ? "16px" : "24px 24px 20px" }}>
                <div style={{ marginBottom: isMobile ? 16 : 24 }}>
                  {q.question_text_ar ? (
                    <>
                      <p style={{ 
                        fontFamily: "'Amiri',serif", fontSize: isMobile ? 18 : 22, fontWeight: 700, color: "#fff", 
                        lineHeight: isMobile ? 2 : 2.2, direction: "rtl", margin: "0 0 10px",
                        wordWrap: "break-word",
                      }}>
                        {stripHtml(q.question_text_ar)}
                      </p>
                      <p style={{ fontSize: isMobile ? 13 : 14, color: "rgba(255,255,255,.6)", lineHeight: 1.7, margin: 0 }}
                        dangerouslySetInnerHTML={{ __html: sanitizeHtml(q.question_text || "") }} />
                    </>
                  ) : (
                    <div style={{ fontSize: isMobile ? 14 : 16, fontWeight: 600, color: "#fff", lineHeight: 1.7 }}
                      dangerouslySetInnerHTML={{ __html: sanitizeHtml(q.question_text || "") }} />
                  )}
                </div>
                    ...
                    {(q.question_type === "true_false"
                      ...
                      return (
                        <button key={optId} onClick={() => saveAnswer(q.id, optId)}
                          style={{                             width: "100%", padding: isMobile ? "12px 14px" : "14px 18px", 
                          ...
                          <span style={{ flex: 1, textAlign: "left" }}>{optTxt}</span>
                        </button>
                      );
                    })}
                  </div>
              ...
              <div style={{ 
                ...
                <button onClick={() => setCurrentIdx(Math.max(0, currentIdx - 1))} disabled={currentIdx === 0}
                  style={{ 
                    ...
                    border: "1.5px solid rgba(255,255,255,.15)", background: "transparent",                     color: "rgba(255,255,255,.7)", cursor: currentIdx === 0 ? "not-allowed" : "pointer", 
                    opacity: currentIdx === 0 ? 0.4 : 1, fontWeight: 700, fontSize: isMobile ? 12 : 13,
                    minHeight: isMobile ? 44 : "auto", justifyContent: "center",
                  }}>
            ...
            <div style={{ padding: isMobile ? "40px 20px" : "60px 20px", textAlign: "center", color: "rgba(255,255,255,.4)" }}>
              <p>No questions found.</p>
              <p style={{ fontSize: 12, marginTop: 8 }}>Questions loaded: {questions.length}</p>
            </div>
          )}
        </div>
        ...
        {/* NAVIGATION GRID */}
        <div style={{ 
          width: isMobile ? "100%" : 220, flexShrink: 0, order: isMobile ? 2 : 1,
          position: isMobile ? "fixed" : "sticky", bottom: isMobile ? 0 : "auto",
          left: isMobile ? 0 : "auto", right: isMobile ? 0 : "auto",
          top: isMobile ? "auto" : 80,
          background: isMobile ? "rgba(11,31,20,.98)" : "rgba(255,255,255,.03)",
          border: isMobile ? "none" : "1px solid rgba(255,255,255,.08)",          borderTop: isMobile ? "1px solid rgba(255,255,255,.08)" : "none",
              ...
              return (
                <button key={idx} className="q-nav-btn" onClick={() => setCurrentIdx(idx)}
                  style={{ 
                    ...
                    outline: isCurrent ? `2px solid ${GOLD}` : "none", minHeight: isMobile ? 36 : "auto",
                  }}>
                  {idx + 1}
                </button>
              );
          ...
          <button onClick={() => setShowConfirm(true)}
            style={{ 
              ...
              minHeight: isMobile ? 44 : "auto", display: "flex", alignItems: "center", justifyContent: "center", gap: 6,            }}>
            <Send size={isMobile ? 12 : 13} /> Submit Exam
          </button>
      ...
      {/* SUBMIT CONFIRMATION */}
      {showConfirm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,.75)", padding: isMobile ? "16px" : 20 }}>
          <div style={{ 
            background: "#0f2d1f", border: "1px solid rgba(201,168,76,.3)", 
            ...
            <div style={{ display: "flex", gap: isMobile ? 10 : 12, flexDirection: isMobile ? "column" : "row" }}>
              <button onClick={() => setShowConfirm(false)}
                style={{ flex: 1, padding: isMobile ? "12px" : "12px", borderRadius: isMobile ? 10 : 12, border: "1.5px solid rgba(255,255,255,.2)", background: "transparent", color: "#fff", fontWeight: 700, cursor: "pointer", minHeight: isMobile ? 44 : "auto" }}>
                Go Back
              </button>
              <button onClick={handleSubmit} disabled={submitting}
                style={{ flex: 1, padding: isMobile ? "12px" : "12px", borderRadius: isMobile ? 10 : 12, border: "none", background: `linear-gradient(135deg,${GOLD},#b8902a)`, color: "#fff", fontWeight: 800, cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? .7 : 1, minHeight: isMobile ? 44 : "auto" }}>
