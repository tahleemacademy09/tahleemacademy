...
export default function HifdhDailySession({ assignment, userId, onClose }: Props) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: WARM, display: "flex", flexDirection: "column", fontFamily: "'Cairo', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Amiri+Quran&family=Amiri:ital,wght@0,400;0,700;1,400&family=Cairo:wght@400;600;700;800&display=swap');
        ...
        .word-partial  { color: #d97706; }
        .word-error    { color: #dc2626; background: #fee2e2; border-radius: 4px; padding: 0 2px; }
        .word-unheard  { color: #1a1208; }
      `}</style>
      ...
      {phase === "reading" && (
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "10px 16px 90px", display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {/* When recording: show Quran page with live word coloring + waveform */}
            {isListening ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {/* Listening header bar */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: W, borderRadius: 12, border: `2px solid ${PASS_COLOR}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontWeight: 800, fontSize: 14, color: G }}>Listening…</span>
                  </div>
                  <div style={{ padding: "4px 14px", borderRadius: 20, background: `${PASS_COLOR}15`, border: `1px solid ${PASS_COLOR}44` }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: PASS_COLOR }}>
                      🔴 {Math.floor(recitedSecs / 60)}:{String(recitedSecs % 60).padStart(2, "0")}
                    </span>
                  </div>
                </div>

                {/* Quran page with live word coloring */}
                {pageAyahs.length > 0 && (() => {
                  const spokenNorm = normalizeAr(liveWords);
                  const spokenWords = spokenNorm.split(" ").filter(Boolean);
                  return (
                    <div className="mushaf-page" style={{
                      borderRadius: 14,
                      border: "2px solid #c9a84c66",
                      padding: "20px 18px 18px",
                      position: "relative",
                      boxShadow: "0 4px 24px rgba(0,0,0,0.10), inset 0 0 0 8px #fdf6e3",
                    }}>
                      <div style={{ position: "absolute", inset: 10, border: "1px solid #c9a84c44", borderRadius: 6, pointerEvents: "none" }} />
                      <p style={{ fontSize: 9, fontWeight: 700, color: "#9CA3AF", margin: "0 0 10px", textTransform: "uppercase", textAlign: "center", letterSpacing: 0.5 }}>
                        Words reveal as you recite — {liveWords ? "🎙 hearing you" : "waiting…"}
                      </p>
                      <div style={{
                        direction: "rtl",
                        ...
                        {pageAyahs.map((ayah, ai) => (
                          <span key={ai}>
                            {ayah.text.split(" ").map((word, wi) => {
                              const wordNorm = normalizeAr(word);
                              // Find if this word has been spoken
                              let status: "correct" | "partial" | "unheard" = "unheard";
                              const exactMatch = spokenWords.some(sw => sw === wordNorm);
                              const prefixMatch = wordNorm.length >= 3 && spokenWords.some(sw =>
                                sw.length >= 3 && sw.slice(0, 3) === wordNorm.slice(0, 3)
                              );
                              const fuzzyMatch = spokenWords.some(sw =>
                                wordNorm.length >= 4 && sw.length >= 4 &&
                                (wordNorm.includes(sw) || sw.includes(wordNorm))
                              );
                              if (exactMatch || fuzzyMatch) status = "correct";
                              else if (prefixMatch) status = "partial";

                              return (
                                <span key={wi} style={{
                                  color: status === "correct" ? "#16a34a"
                                       : status === "partial" ? "#d97706"
                                       : "#1a0a00",
                                  background: status === "correct" ? "#dcfce733"
                                            : status === "partial" ? "#fef3c733"
                                            : "transparent",
                                  borderRadius: 3,
                                  padding: "0 1px",
                            ...
                            <span style={{
                              ...
                              border: "1.5px solid #c9a84c", background: "#fdf6e3",
                              fontSize: 10, color: "#c9a84c", fontFamily: "'Amiri', serif",
                      ...
                      <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 10 }}>
                        {[["#16a34a","✓ Correct"],["#d97706","~ Partial"],["#9CA3AF","◌ Not yet"]].map(([c,l]) => (
              ...
              fetchingPage ? (
                ...
                <div className="mushaf-page" style={{
                  ...
                  position: "relative",
                  boxShadow: "0 4px 24px rgba(0,0,0,0.10), inset 0 0 0 8px #fdf6e3",
                  outline: "1px solid #c9a84c22",
                  outlineOffset: "8px",
                }}>
                  {/* Inner decorative border like a real mushaf */}
                  <div style={{ position: "absolute", inset: 10, border: "1px solid #c9a84c44", borderRadius: 6, pointerEvents: "none" }} />
                  <p style={{ fontSize: 10, fontWeight: 700, color: "#9CA3AF", margin: "0 0 12px", textTransform: "uppercase", letterSpacing: 0.6, textAlign: "center" }}>
                    {pageAyahs[0]?.surah?.englishName} · صفحة {pagesToRevise[pageIdx]}
                  </p>
                  {/* Authentic mushaf-style: continuous flowing text, verse numbers inline, justified */}
                  <div style={{
                    direction: "rtl",
                    fontFamily: "'Amiri Quran', 'Amiri', serif",
                    fontSize: 22,
                    ...
                    {pageAyahs.map((a, i) => (
                      <span key={i}>
                        {a.text}
                        {/* Circular verse-end ornament matching image 3 */}
                        <span style={{
                          display: "inline-flex",
                          alignItems: "center",
                  ...
                  <p style={{ fontSize: 10, color: "#9CA3AF", margin: "12px 0 0", textAlign: "center" }}>
                    Read carefully, then tap <strong>Start Reciting</strong> and recite from memory
                  </p>
                </div>
              ) : (
            ...
            {pageAyahs.length > 0 && (() => {
              ...
              return (
                <div style={{ background: W, borderRadius: 14, border: "1.5px solid #E5E7EB", padding: "12px 14px" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, direction: "rtl", lineHeight: 2.2 }}>
                    {allWords.map(({ display, norm }, i) => {
                      ...
                      return (
                        <span key={i} style={{
                          padding: "5px 12px", borderRadius: 8, fontSize: 18,
                          fontFamily: "'Amiri Quran','Amiri',serif",
              ...
              return (
                <div style={{ background: W, borderRadius: 16, border: `1px solid ${BRD}`, overflow: "hidden" }}>
                  <div style={{ padding: "14px", background: `${G}0a`, borderBottom: `1px solid ${BRD}` }}>
                    ...
                    <p style={{ fontSize: 20, direction: "rtl", fontFamily: "'Amiri Quran','Amiri',serif", color: INK, lineHeight: 2.6, margin: 0 }}>{q.prompt}</p>
                  </div>
