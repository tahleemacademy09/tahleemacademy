...
// The reference text for Al-Fatiha (used for comparison)
const AL_FATIHA_TEXT = "بسم الله الرحمن الرحيم الحمد لله رب العالمين الرحمن الرحيم مالك يوم الدين إياك نعبد وإياك نستعين اهدنا الصراط المستقيم صراط الذين أنعمت عليهم غير المغضوب عليهم ولا الضالين";
...
const RecitationTest = () => {
  ...
  const runAIScoring = async (path: string) => {
    ...
    try {
      ...
      if (!audioData || !GROQ_KEY) {
        // Demo mode — simulate a score
        await new Promise(r => setTimeout(r, 2000));
        const demoScore = Math.floor(Math.random() * 30) + 65; // 65–95%
      ...
      const refWords  = AL_FATIHA_TEXT.replace(/[^\u0600-\u06FF\s]/g, "").trim().split(/\s+/);
      const gotWords  = transcript.replace(/[^\u0600-\u06FF\s]/g, "").trim().split(/\s+/);
      let matched = 0;
      ...
      setAiScore(score);
      await saveAIScore(score, transcript);

    } catch (e: any) {
      // Fallback to demo score on error
      const fallback = 72;
      setAiScore(fallback);
        ...
        <div style={{ width:"100%", maxWidth:560, background:"#fff", borderRadius:24, boxShadow:"0 24px 80px rgba(0,0,0,.3)", overflow:"hidden", animation:"fadeUp .4s ease" }}>
          ...
          <div style={{ background:`linear-gradient(135deg,${G},${GM})`, padding:"20px 24px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:0 }}>
              {[
                { n:1, icon:<Mic size={14}/>,      label:"Record" },
                { n:2, icon:<Star size={14}/>,     label:"AI Score" },
                { n:3, icon:<Video size={14}/>,    label:"Live Session" },
              ].map((s,i) => (
              ...
              <div>
                <div style={{ textAlign:"center", marginBottom:20 }}>
                  ...
                  <div style={{ fontSize:13, color:"#666", lineHeight:1.6 }}>
                    Recite <strong>Surah Al-Fatiha</strong> clearly into your microphone.<br/>
                    Speak at your natural pace — do not rush.
                  </div>
                </div>

                {/* Reference text */}
                <div style={{ background:"#FFFBEB", borderRadius:14, padding:"14px 16px", border:"1px solid #F9D46A", marginBottom:20, textAlign:"center" }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#92400E", marginBottom:8, textTransform:"uppercase", letterSpacing:.5 }}>Recite This Surah</div>
                  <div style={{ fontSize:18, fontFamily:"serif", direction:"rtl", lineHeight:2, color:G }}>
                ...
                <div style={{ background:"#F0FDF4", borderRadius:12, padding:"12px 16px", border:"1px solid #86EFAC", marginBottom:20 }}>
                  ...
                  {["Find a quiet room with no background noise","Hold phone 15–20cm from your mouth","Recite clearly and at your normal pace","Do not stop in the middle — complete the full Surah"].map((t,i) => (
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                    ...
                    {sessionDate && (
                      <div>
                        ...
                        <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                          {["08:00","10:00","12:00","14:00","16:00","18:00","20:00","21:00"].map(t => {
