...
/* ── Bilingual field renderer ──────────────────────────────────
   ...
   language — whichever the student's global app-language toggle (a
   per-device localStorage setting, defaulting to English for everyone)
   happened to be set to. That meant Arabic text a teacher wrote was
   invisible to virtually every student, since almost none had switched
   the app's whole UI language just to read one assignment.

   This renders whichever language(s) are actually filled in, together:
     • only English filled in → single ltr block
     • only Arabic filled in  → single rtl block
     • both filled in         → stacked, with the Arabic block visually
       indented/separated from the English one (a left border + indent)
       so the two scripts don't run together
   `dir="auto"` on the English block lets the browser correctly handle
   the rare case where a teacher typed occasional Arabic words inline. */
function Bilingual({ en, ar, style, arStyle }: {
  en?: string | null; ar?: string | null;
  style?: React.CSSProperties; arStyle?: React.CSSProperties;
}) {
  const hasEn = !!en?.trim();
  const hasAr = !!ar?.trim();
  if (!hasEn && !hasAr) return null;
  if (hasEn && !hasAr) return <div dir="auto" style={style}>{en}</div>;
  if (hasAr && !hasEn) return <div dir="rtl" style={{ ...style, ...arStyle }}>{ar}</div>;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div dir="auto" style={style}>{en}</div>
      <div dir="rtl" style={{
        ...style, ...arStyle,
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                        <Bilingual en={a.title} ar={a.title_ar} style={{ fontSize: 15, fontWeight: 800, color: TXT }} />
      ...
      <div style={{ width: "100%", maxWidth: 600, background: "#fff", borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden" }}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
          <Field label={t("Title", "العنوان")}>
            <input value={title} onChange={e => setTitle(e.target.value)} dir="auto" placeholder={t("e.g. Tajweed Chapter 3 Exercises", "مثال: تمارين التجويد الفصل 3")} style={inputStyle} />
          </Field>
          ...
          <Field label={t("Instructions", "التعليمات")}>
            <textarea value={desc} onChange={e => setDesc(e.target.value)} dir="auto" rows={4} style={{ ...inputStyle, height: "auto", resize: "vertical" }} />
          </Field>
          ...
          <Field label={t("Question", "السؤال")}>
            <textarea value={question} onChange={e => setQuestion(e.target.value)} dir="auto" rows={4} placeholder={t("What should the student answer?", "ما الذي يجب على الطالب الإجابة عنه؟")} style={{ ...inputStyle, height: "auto", resize: "vertical" }} />
          </Field>
      ...
      <div style={{ width: "100%", maxWidth: 640, background: "#fff", borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden" }}
        ...
        <div style={{ padding: "12px 20px 14px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <Bilingual en={a.title} ar={a.title_ar} style={{ fontSize: 18, fontWeight: 900, color: TXT, margin: 0 }} />
          </div>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 10, border: `1px solid ${BORDER}`, background: "#f4f4f4", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <X style={{ width: 15, height: 15, color: "#888" }} />
          </button>
...
function StudentAssignmentList({ subjectId }: { subjectId?: string }) {
  ...
  const filtered = assignments.filter(a => {
    ...
    if (search) {
      ...
      const matchesEn = a.title?.toLowerCase().includes(q);
      const matchesAr = a.title_ar?.toLowerCase().includes(q);
        ...
        <div style={{ padding: "12px 20px 14px", borderBottom: `1px solid ${BORDER}` }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <Bilingual en={a.title} ar={a.title_ar} style={{ fontSize: 18, fontWeight: 900, color: TXT, margin: 0, lineHeight: 1.2 }} />
              {a.deadline && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {(a.description || a.description_ar) && (
                <div style={{ background: "#f8fafb", border: `1px solid ${BORDER}`, borderRadius: 14, padding: "14px 16px" }}>
                  ...
                  <Bilingual en={a.description} ar={a.description_ar}
                    style={{ fontSize: 14, color: TXT, lineHeight: 1.65, whiteSpace: "pre-wrap" }} />
                </div>
              )}
              {(a.question || a.question_ar) && (
                <div style={{ background: `${G}08`, border: `1px solid ${G}20`, borderRadius: 14, padding: "14px 16px" }}>
                  ...
                  <Bilingual en={a.question} ar={a.question_ar}
                    style={{ fontSize: 14, color: TXT, lineHeight: 1.65, whiteSpace: "pre-wrap", fontWeight: 600 }} />
                </div>
              )}
