/*  src/pages/student/Onboarding.tsx
    ...
    ✅ SESSION PERSISTENCE: Form data + current step are saved to localStorage
       so if the window is closed and reopened the user returns to exactly where they left off.
*/
...
const saveProgress = (uid: string, data: Record<string, any>) => {
  try { localStorage.setItem(LS_KEY(uid), JSON.stringify(data)); } catch (_) {}
};
const loadProgress = (uid: string): Record<string, any> | null => {
  try { const r = localStorage.getItem(LS_KEY(uid)); return r ? JSON.parse(r) : null; } catch (_) { return null; }
};
const clearProgress = (uid: string) => {
  try { localStorage.removeItem(LS_KEY(uid)); } catch (_) {}
};
...
const Onboarding = () => {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    if (saved.step > 1)   { setStep(saved.step); setShowResumeNotice(true); }
    ...
    if (saved.quranLevel)   setQuranLevel(saved.quranLevel);
    if (saved.memorized)    setMemorized(saved.memorized);
    if (saved.yearsStudy)   setYearsStudy(saved.yearsStudy);
    if (saved.tajweed)      setTajweed(saved.tajweed);
    if (saved.prevTeacher)  setPrevTeacher(saved.prevTeacher);
    if (saved.arabic)       setArabic(saved.arabic);
    if (saved.islamic)      setIslamic(saved.islamic);
    if (saved.subjects)     setSubjects(saved.subjects);
    if (saved.goals)        setGoals(saved.goals);
    if (saved.hours)        setHours(saved.hours);
    if (saved.timePrefer)   setTimePrefer(saved.timePrefer);
  ...
  useEffect(() => {
    ...
    saveProgress(user.id, {
      step, phone, dob, gender, country, city, occupation,
      quranLevel, memorized, yearsStudy, tajweed, prevTeacher,
      arabic, islamic, subjects, goals, hours, timePrefer, device, heardFrom, notes,
    });
  }, [user, step, phone, dob, gender, country, city, occupation,
      quranLevel, memorized, yearsStudy, tajweed, prevTeacher,
      arabic, islamic, subjects, goals, hours, timePrefer, device, heardFrom, notes]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      await supabase.from("tasjeel_progress" as any).update({
        current_step: "exam", updated_at: new Date().toISOString(),
      ...
      try {
        const { data: existing } = await supabase.from("exam_attempts").select("id, status")
          .eq("exam_id", ENTRANCE_EXAM_ID).eq("user_id", user.id).eq("status", "in_progress").maybeSingle();
        if (existing) { navigate(`/student/entrance-exam/${existing.id}`); return; }
        const { data: newAttempt, error: attemptErr } = await supabase.from("exam_attempts").insert({
          exam_id: ENTRANCE_EXAM_ID, user_id: user.id, status: "in_progress",
          started_at: new Date().toISOString(),
        }).select("id").single();
        if (attemptErr || !newAttempt) { navigate("/student/exams"); return; }
  ...
  const resetForm = () => {
    if (!user) return;
    clearProgress(user.id);
    setStep(1); setPhone(""); setDob(""); setGender(""); setCountry(""); setCity("");
    setOccupation(""); setQuranLevel(""); setMemorized([]); setYearsStudy("");
    setTajweed(""); setPrevTeacher(""); setArabic(""); setIslamic(""); setSubjects([]);
    setGoals([]); setHours(""); setTimePrefer(""); setDevice(""); setHeardFrom(""); setNotes("");
        ...
        <div style={{ width:"100%", maxWidth:560, background:"#fff", borderRadius:24, boxShadow:"0 24px 80px rgba(0,0,0,.3)", overflow:"hidden", animation:"fadeUp .4s ease" }}>
          ...
          {showResumeNotice && (
            <div style={{ background:"#FFF8E1", padding:"12px 20px", borderBottom:"1px solid #F9D46A", display:"flex", alignItems:"center", gap:10, animation:"slideDown .3s ease" }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#92400E" }}>Welcome back! Progress restored.</div>
                <div style={{ fontSize:12, color:"#78350F" }}>You left off at Step {step}. Continue from where you stopped.</div>
              </div>
              <button onClick={resetForm} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#92400E", background:"rgba(245,158,11,.1)", border:"1px solid #F9D46A", borderRadius:8, padding:"5px 10px", cursor:"pointer", fontWeight:600, whiteSpace:"nowrap" as const }}>
          ...
          <div style={{ background:`linear-gradient(135deg,${G},${GM})`, padding:"20px 24px" }}>
            <div style={{ fontSize:11, color:"rgba(255,255,255,.5)", textTransform:"uppercase", letterSpacing:1, marginBottom:4 }}>Step {step} of {TOTAL}</div>
          ...
          <div style={{ padding:"24px 24px 28px" }}>
            ...
            {step === 1 && (
              <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Phone <span style={{color:"#ef4444"}}>*</span></label>
                    <input value={phone} onChange={e=>setPhone(e.target.value)} {...f("phone")} style={inputSt(foc==="phone")} placeholder="+234 800 000 0000" type="tel" /></div>
                  <div><label style={lbl}>Date of Birth <span style={{color:"#ef4444"}}>*</span></label>
                    <input value={dob} onChange={e=>setDob(e.target.value)} {...f("dob")} style={inputSt(foc==="dob")} type="date" /></div>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Gender <span style={{color:"#ef4444"}}>*</span></label>
                    <Sel val={gender} onChange={setGender} opts={["Male","Female"]} /></div>
                  <div><label style={lbl}>Occupation</label>
                    <Sel val={occupation} onChange={setOccupation} opts={["Student","Working professional","Business owner","Homemaker","Retired","Other"]} /></div>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Country <span style={{color:"#ef4444"}}>*</span></label>
                    <input value={country} onChange={e=>setCountry(e.target.value)} {...f("country")} style={inputSt(foc==="country")} placeholder="e.g. Nigeria" /></div>
                  <div><label style={lbl}>City / State</label>
                    <input value={city} onChange={e=>setCity(e.target.value)} {...f("city")} style={inputSt(foc==="city")} placeholder="e.g. Lagos" /></div>
                </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
                <div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {[["none","Cannot read Arabic letters yet"],["letters","Know the letters but cannot read words"],["qaida","On Noorani Qaida / basic reader"],["slow","Can read Quran slowly with mistakes"],["fluent","Can read Quran fluently with Tajweed"],["memorising","Currently memorising (Hifz)"],["hafiz","Already a Hafiz (memorised full Quran)"]].map(([v,l]) => <Radio key={v} name="quran" val={v} checked={quranLevel===v} onChange={() => setQuranLevel(v)} label={l} />)}
                  </div>
                </div>
                <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Years studying Quran</label><Sel val={yearsStudy} onChange={setYearsStudy} opts={["Less than 1 year","1–2 years","3–5 years","5–10 years","More than 10 years"]} /></div>
                  <div><label style={lbl}>Tajweed knowledge</label><Sel val={tajweed} onChange={setTajweed} opts={["None","Basic rules only","Intermediate","Advanced / Formal study"]} /></div>
                </div>
                <div><label style={lbl}>Previous teacher / institute (if any)</label>
                  <input value={prevTeacher} onChange={e=>setPrevTeacher(e.target.value)} {...f("prev")} style={inputSt(foc==="prev")} placeholder="e.g. Sheikh Abdullahi, Al-Noor Institute" /></div>
              </div>
            )}
            ...
            {step === 3 && (
              <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
                <div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {[["none","No Arabic knowledge"],["letters","Know letters and basic sounds"],["beginner","Can read but don't understand meaning"],["intermediate","Basic grammar (Nahw/Sarf) understanding"],["advanced","Advanced — can read and understand texts"]].map(([v,l]) => <Radio key={v} name="arabic" val={v} checked={arabic===v} onChange={() => setArabic(v)} label={l} />)}
                  </div>
                </div>
                <div><label style={lbl}>Islamic Studies Knowledge</label>
                  <Sel val={islamic} onChange={setIslamic} opts={["Very basic — pillars only","Intermediate — some Fiqh & Aqeedah","Advanced — studied with a scholar","Self-taught — read extensively"]} /></div>
                <div>
            ...
            {step === 4 && (
              <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Study hours per day</label><Sel val={hours} onChange={setHours} opts={["Less than 30 min","30 min – 1 hour","1–2 hours","2–3 hours","More than 3 hours"]} /></div>
                  <div><label style={lbl}>Preferred time to learn</label><Sel val={timePrefer} onChange={setTimePrefer} opts={["Early morning (Fajr)","Morning","Afternoon","Evening","Night (after Isha)","Flexible"]} /></div>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  <div><label style={lbl}>Primary device</label><Sel val={device} onChange={setDevice} opts={["Mobile phone","Tablet","Laptop / PC","Multiple devices"]} /></div>
                  <div><label style={lbl}>How did you hear about us?</label><Sel val={heardFrom} onChange={setHeardFrom} opts={["Social media","Friend / Family","WhatsApp","Google","Mosque / Islamic centre","Other"]} /></div>
                </div>
                <div><label style={lbl}>Anything else for your teacher to know?</label>
                  <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} {...f("notes")}
                    style={{ ...inputSt(foc==="notes"), resize:"none", lineHeight:1.5 }}
                    placeholder="Health conditions, learning difficulties, special requests…" /></div>
            ...
            <div style={{ display:"flex", gap:10, marginTop:24 }}>
              ...
              <button type="button" onClick={next} disabled={saving}
                ...
                {saving ? <><Loader2 style={{ width:18, height:18, animation:"spin .8s linear infinite" }} /> Saving…</>
                  : step === TOTAL ? <><CheckCircle2 size={18} /> Submit & Start Exam</>
