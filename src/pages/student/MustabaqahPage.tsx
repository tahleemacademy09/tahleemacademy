...
export default function MustabaqahPage() {
  ...
  const callParticipant = async (p: Participant) => {
    ...
    const { att } = await supabase.from("musabaqah_attempts" as any).insert({
  ...
  const joinCompetition = async () => {
    ...
    const { existing } = await supabase.from("musabaqah_participants" as any).select("*").eq("competition_id", (comp as Competition).id).eq("user_id", user?.id).single();
    if (existing) {
    ...
    const { participant } = await supabase.from("musabaqah_participants" as any).insert({
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {shown.map((c, i) => (
                <div key={c.id} onClick={() => openComp(c)}
                  ...
                  style={{                    borderRadius:18, padding:"18px 20px", cursor:"pointer",
          ...
          <div style={{ padding:"12px 16px 0" }}>
            <div className="glass-card" style={{ borderRadius:18, padding:"16px 20px", marginBottom:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div>
                  <div style={{ color:GOLD, fontWeight:800, fontSize:14 }}>Following Live</div>                  <div style={{ color:"rgba(255,255,255,0.4)", fontSize:12, marginTop:2 }}>
