...
export default function MustabaqahPage() {
  ...
  const [rosterEditScores, setRosterEditScores] = useState<Record<string,string>>({});
  const [rosterCallStage, setRosterCallStage] = useState<Record<string,string>>({});
  const [showScorePanel, setShowScore]     = useState(false);
  // ── Manual "stuck" recovery: lets anyone force a resync of their exact
  //    stage/status from the DB instead of waiting on broadcast/polling.
  const [resyncing,      setResyncing]     = useState(false);
  // ── Stage review/correction: the judge can step Back to an already-scored
  //    stage of the current participant to fix a mistake, then Next back to
  //    where they left off. stageFrontier is the furthest stage reached so
  //    far for this participant — Back/Next are only allowed within it.
  const [stageFrontier,  setStageFrontier] = useState(1);
  ...
  // ── Manual "stuck" recovery ─────────────────────────────────────────
  // Broadcasts and polling usually keep everyone in sync, but a dropped
  // connection or a missed event can still leave someone stuck (most often
  // a participant stuck at "picking number"). This lets anyone force a fresh
  // pull from the DB and land back in exactly the right spot — no full page
  // reload needed.
  const forceResync = useCallback(async () => {
    ...
    try {
      const { data: compRow } = await supabase
        .from("musabaqah_competitions" as any).select("*").eq("id", comp.id).single();
      if (compRow) {
        setCompetition(c => c ? { ...c, ...(compRow as any) } : (compRow as any));
      }
      await Promise.all([loadParticipants(), loadAttempts(), loadJudgeScores()]);

      const cfg = ((compRow as any)?.scope_config || {}) as any;
      const mine = myParticipantRef.current;
      if (mine) {
        if (mine.status === "called" && cfg.current_tiles?.length && cfg.current_called_id === mine.id) {
          setStageTiles(cfg.current_tiles);
          setShowTilePicker(true);
          setPickerParticipantId(mine.id);
          if (cfg.current_stage_num) setPickerStage(cfg.current_stage_num);
        } else if (mine.status === "reciting" && !pickedTile) {
          const stage = cfg.current_stage_num || pickerStage;
          const stageAttempts = attempts.filter(a => a.participant_id === mine.id && a.stage_number === stage);
          const latest = stageAttempts[stageAttempts.length - 1];
          if (latest) {
            setCurAttempt(latest);
            setBellCount(latest.bell_count ?? 0);
      ...
      toast({ title: "🔄 Synced", description: "You're up to date." });
    } catch {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[loadParticipants, loadAttempts, loadJudgeScores, pickedTile, attempts, pickerStage]);
  ...
  useEffect(()=>{
    ...
    const ch = supabase.channel(`musabaqah:${competition.id}`,{config:{broadcast:{ack:false}}})
      .on("broadcast",{event:"BELL"},({payload}:any)=>{
        ...
        // Minor errors stay invisible to reciters — no flash, no sound, no alert.
        // Judges and observers still get notified of every bell (minor + major);
        // a reciter only ever feels/hears a major error.
        const isReciterClient = !isJudge && !isObserver;
        if (isReciterClient && payload.severity==="minor") return;
        setBellFlash(true);
  ...
  const callParticipant = async (p:Participant, startStage:number=1) => {
    if (!competition) return;
    const stage = Math.max(1, Math.min(startStage, competition.total_stages));
    ...
    // Normally starts at stage 1 (all stages in one sitting), but a judge can
    // call someone in directly at a later stage — e.g. resuming a participant
    // who already has recorded scores for stages 1-3 and should continue at 4.
    setActiveParticipantStage(stage);
    setStageFrontier(stage);
    setPickerStage(stage);
    const tiles = buildTiles(competition, stage);
    ...
    broadcast("TILES_SHOWN",{tiles,stage,picker_participant_id:p.id});
    playCalled();
    ...
    setTimeout(()=>broadcast("TILES_SHOWN",{tiles,stage,picker_participant_id:p.id}), 800);
  };
  ...
  const finalizeAttemptScore = async (attempt: Attempt, total: number, breakdown: Record<string,number>, judgeCount: number) => {
    ...
    try {
      ...
      if (hasMoreStages) {
        ...
        setActiveParticipantStage(1); setStageFrontier(1); setMyScoreSubmitted(false);
  ...
  /** Judge steps Back/Next between stages already reached (1..stageFrontier)
   *  for the currently active participant — e.g. to revisit stage 2 and fix
   *  a scoring mistake before returning to where they left off. Pulls the
   *  recorded attempt for that stage back into the score panel for editing.
   *  Never touches participant status or auto-advances — that only happens
   *  through the normal Submit Score flow. */
  const reviewStage = (stageNum: number) => {
    if (!activeP || !competition) return;
    const clamped = Math.max(1, Math.min(stageNum, stageFrontier));
    if (clamped === activeParticipantStage) return;
    setActiveParticipantStage(clamped);
    setPickerStage(clamped);
    setMyScoreSubmitted(false);
    const stageAttempts = attempts.filter(a => a.participant_id === activeP.id && a.stage_number === clamped);
    const latest = stageAttempts[stageAttempts.length - 1];
    if (latest) {
      setCurAttempt(latest);
      setBellCount(latest.bell_count ?? 0);
      setMinorCount((latest.score_breakdown as any)?.__minor ?? 0);
      setMajorCount((latest.score_breakdown as any)?.__major ?? 0);
      setJudgeComment(latest.judge_comment || "");
      if (competition.use_criteria_scoring) {
        const bd = (latest.score_breakdown || {}) as any;
        setScoreBreak({ tajweed: String(bd.tajweed ?? ""), memorize: String(bd.memorize ?? ""), fluency: String(bd.fluency ?? ""), voice: String(bd.voice ?? "") });
      } else {
        setScoreBreak(s => ({ ...s, tajweed: String(latest.judge_score ?? "") }));
      }
      const tile: Tile = latest.surah_number && latest.ayah_number
        ? { num: clamped, label: latest.scope_label, labelAr: latest.scope_label_ar, surah: latest.surah_number, ayah: latest.ayah_number,
            surahName: SURAHS.find(s => s.n === latest.surah_number)?.en ?? "", surahAr: SURAHS.find(s => s.n === latest.surah_number)?.ar ?? "" }
        : { num: clamped, label: latest.scope_label, labelAr: latest.scope_label_ar, surah: 0, ayah: 0, surahName: "", surahAr: "" };
      setPickedTile(tile);
      if (tile.surah > 0) fetchAyah(tile.surah, tile.ayah);
      setShowScore(true);
      setTilePickerCollapsed(true);
    } else {
      // No recorded attempt for this stage (shouldn't normally happen inside the frontier)
      setCurAttempt(null); setPickedTile(null); setAyahText(null); setShowScore(false);
      setBellCount(0); setMinorCount(0); setMajorCount(0);
      setScoreBreak({ tajweed: "", memorize: "", fluency: "", voice: "" }); setJudgeComment("");
    }
  };

  /** Saves a corrected score for a stage being reviewed (activeParticipantStage
   *  < stageFrontier) — updates just that stage's number and recalculates the
   *  participant's total from all stage scores. Does NOT advance the stage or
   *  change participant status, since correction is separate from progression. */
  const saveStageCorrection = async () => {
    if (!activeP || !competition || !currentAttempt) return;
    ...
    try {
      ...
      total = Math.max(0, Math.round((total - errorPenalty) * 10) / 10);

      await supabase.from("musabaqah_attempts" as any).update({
        judge_score: total, score_breakdown: { ...breakdown, __minor: minorCount, __major: majorCount },
        judge_comment: judgeComment, bell_count: bellCount, status: "scored",
      ...
      toast({ title: `✅ Stage ${activeParticipantStage} corrected: ${total} pts`, description: "Total recalculated from all stages." });
    } catch (err: any) {
      toast({ title: "Save failed", description: err.message, variant: "destructive" });
    } finally {
  ...
  /** Manual admin correction from the Roster — edit any participant's score for
   *  any stage (even after they've completed), independent of the live scoring
  ...
  const correctRosterStageScore = async (p: Participant, stageNum: number, newScore: number) => {
    ...
    toast({ title: `Stage ${stageNum} corrected`, description: `${p.participant_name}: ${newScore} pts (total recalculated)` });
  };

  /** Manual admin override — mark a participant as Called (pulls them into the
   *  active slot, same as tapping Call) or Done (marks all stages complete
   *  without changing already-recorded scores). Useful when the normal flow
   *  gets stuck and the judge needs to force a participant's status directly. */
  const setParticipantStatusManually = async (p: Participant, status: "called" | "completed" | "waiting", stage?: number) => {
    if (!competition) return;
    if (status === "called") {
      await callParticipant(p, stage ?? 1);
      return;
    }
    await supabase.from("musabaqah_participants" as any).update({ status } as any).eq("id", p.id);
    setParticipants(ps => ps.map(x => x.id === p.id ? { ...x, status } : x));
    if (activeP?.id === p.id) {
      setActiveP(null); setShowTilePicker(false); setStageTiles([]); setPickedTile(null); setAyahText(null);
      setShowScore(false); setCurAttempt(null); setTimerActive(false); setTimerExpired(false); setPickerParticipantId(null);
  ...
  return (
    <div style={{minHeight:"100vh",maxHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:"Cairo,sans-serif",overflow:"hidden",position:"relative",background:"#050f08"}}>
      <GlobalStyles/><IslamicBackground/>
      <BellFlash visible={bellFlash} count={(isJudge||isObserver)?bellCount:majorCount}/>
      <CountdownOverlay value={countdownValue}/>
      <StopFlash visible={stopFlash}/>

      {/* Manual resync — for reciters stuck at picking number or any other
          state that didn't update on its own. */}
      {!isJudge&&!isObserver&&myParticipant&&(
        <button onClick={forceResync} disabled={resyncing} title="Refresh — resync to exactly where you stopped"
          style={{position:"fixed",bottom:20,right:16,zIndex:45,width:42,height:42,borderRadius:"50%",background:"rgba(10,20,15,.9)",border:`1.5px solid ${GOLD}55`,display:"flex",alignItems:"center",justifyContent:"center",cursor:resyncing?"not-allowed":"pointer",boxShadow:"0 4px 16px rgba(0,0,0,.4)",opacity:resyncing?.7:1}}>
          <RefreshCw size={17} color={GOLD} style={resyncing?{animation:"spin 1s linear infinite"}:undefined}/>
        </button>
      )}
      ...
      {activeP?.status==="reciting" && timerActive && (
        <div style={{flexShrink:0,padding:"7px 12px",background:"rgba(4,12,6,.98)",borderBottom:"1px solid rgba(34,197,94,.12)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
          ...
          {/* Reciters only ever see major errors — minor errors are tracked for
              scoring but never surfaced as an "error" to the person reciting. */}
          {(isJudge||isObserver)
            ? (bellCount>0&&<div style={{color:RED,fontSize:11,fontWeight:700,flexShrink:0}}>🔔×{bellCount}</div>)
            : (majorCount>0&&<div style={{color:RED,fontSize:11,fontWeight:700,flexShrink:0}}>🔔×{majorCount}</div>)}
        </div>
      )}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {showScorePanel&&(
                  <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:"13px"}}>
                    ...
                    {/* Back/Next — step between already-reached stages of this participant
                        to fix a mistake on an earlier stage, then return to where you left off. */}
                    {stageFrontier>1&&(
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:9}}>
                        <button onClick={()=>reviewStage(activeParticipantStage-1)} disabled={activeParticipantStage<=1} style={{flex:1,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,padding:"7px 0",cursor:activeParticipantStage<=1?"not-allowed":"pointer",color:activeParticipantStage<=1?"rgba(255,255,255,.2)":"#fff",fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                          <ArrowLeft size={12}/> Back
                        </button>
                        <span style={{color:"rgba(255,255,255,.35)",fontSize:10,fontWeight:700,flexShrink:0}}>Stage {activeParticipantStage} / {stageFrontier}</span>
                        <button onClick={()=>reviewStage(activeParticipantStage+1)} disabled={activeParticipantStage>=stageFrontier} style={{flex:1,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,padding:"7px 0",cursor:activeParticipantStage>=stageFrontier?"not-allowed":"pointer",color:activeParticipantStage>=stageFrontier?"rgba(255,255,255,.2)":"#fff",fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                          Next <ArrowRight size={12}/>
                        </button>
                      </div>
                    )}
                    {activeParticipantStage<stageFrontier&&(
                      <div style={{background:"rgba(167,139,250,.1)",border:"1px solid rgba(167,139,250,.3)",borderRadius:8,padding:"7px 10px",marginBottom:9,color:"#a78bfa",fontSize:11,fontWeight:700}}>
                        🕓 Reviewing a completed stage — edit and Save Correction below, then tap Next to return to Stage {stageFrontier}.
                      </div>
                    )}
                    ...
                    {activeParticipantStage===stageFrontier&&presentJudges.length>1&&(
                    ...
                    {activeParticipantStage<stageFrontier ? (
                      // ── Reviewing/correcting an already-scored stage — a direct save,
                      //    completely separate from the multi-judge live submit flow below. ──
                      <button onClick={saveStageCorrection} disabled={savingCorrection} style={{width:"100%",background:savingCorrection?"rgba(167,139,250,.4)":"linear-gradient(135deg,#a78bfa,#8b5cf6)",color:"#fff",border:"none",borderRadius:9,padding:"11px",cursor:savingCorrection?"not-allowed":"pointer",fontWeight:800,fontFamily:"Cairo,sans-serif",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
                        {savingCorrection?<><span style={{animation:"spin .8s linear infinite",display:"inline-block"}}>⏳</span> Saving…</>:<><CheckCircle size={13}/> Save Correction</>}
                      </button>
                    ) : judgeSubrole==="marker" ? (
                    ...
                    {activeParticipantStage===stageFrontier&&myScoreSubmitted && presentJudges.length>1 && (judgeScores[currentAttempt?.id||""]||[]).length < presentJudges.length && (
            ...
            {judgeTab==="roster"&&(
              <div>
                ...
                {rosterMode==="list"
                  ? participants.filter(p=>p.status!=="pending").map(p=>{
                      const isActive=p.id===activeP?.id;
                      const isExpanded=expandedRosterId===p.id;
                      return(
                        <div key={p.id} style={{marginBottom:4}}>
                          <div style={{background:isActive?"rgba(201,168,76,.06)":"rgba(255,255,255,.02)",border:`1px solid ${isActive?`${GOLD}33`:"rgba(255,255,255,.05)"}`,borderRadius:isExpanded?"8px 8px 0 0":8,padding:"7px 9px",display:"flex",alignItems:"center",gap:6}}>
                            ...
                            <button onClick={()=>setExpandedRosterId(isExpanded?null:p.id)} title="Correct scores / status" style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.12)",borderRadius:6,padding:"3px 6px",cursor:"pointer",color:"rgba(255,255,255,.5)",flexShrink:0}}>
                            {isExpanded?"▴":"⚙️"}
                            </button>
                          </div>
                          {isExpanded&&(
                            <div style={{background:"rgba(0,0,0,.25)",border:"1px solid rgba(255,255,255,.08)",borderTop:"none",borderRadius:"0 0 8px 8px",padding:"9px"}}>
                              <div style={{color:"rgba(255,255,255,.35)",fontSize:9,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>Per-stage score (edit & save)</div>
                              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:9}}>
                                {Array.from({length:competition.total_stages},(_,i)=>{
                                  const s=i+1;
                                  const key=`${p.id}:${s}`;
                                  const savedVal=(p.stage_scores||{})[s];
                                  const val = rosterEditScores[key] ?? (savedVal!==undefined?String(savedVal):"");
                                  return(
                                    <div key={s} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                                      <span style={{color:"rgba(255,255,255,.3)",fontSize:9}}>Stage {s}</span>
                                      <input type="number" value={val} onChange={e=>setRosterEditScores(m=>({...m,[key]:e.target.value}))} placeholder="-" style={{width:54,background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.15)",borderRadius:6,padding:"5px 4px",color:"#fff",fontSize:12,textAlign:"center"}}/>
                                      <button onClick={()=>{
                                        const n=Number(rosterEditScores[key] ?? savedVal ?? 0)||0;
                                        correctRosterStageScore(p, s, n);
                                        setRosterEditScores(m=>{ const c={...m}; delete c[key]; return c; });
                                      }} style={{background:`${GOLD}18`,border:`1px solid ${GOLD}44`,borderRadius:6,padding:"3px 8px",cursor:"pointer",color:GOLD,fontWeight:700,fontSize:9,fontFamily:"Cairo,sans-serif"}}>Save</button>
                                    </div>
                                  );
                                })}
                              </div>
                              <div style={{color:"rgba(255,255,255,.35)",fontSize:9,fontWeight:700,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>Manual status</div>
                              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                                <span style={{color:"rgba(255,255,255,.4)",fontSize:10,flexShrink:0}}>Call at stage:</span>
                                <input type="number" min={1} max={competition.total_stages}
                                  value={rosterCallStage[p.id] ?? String(Math.min(competition.total_stages, Object.keys(p.stage_scores||{}).length+1))}
                                  onChange={e=>setRosterCallStage(m=>({...m,[p.id]:e.target.value}))}
                                  style={{width:48,background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.15)",borderRadius:6,padding:"5px 4px",color:"#fff",fontSize:12,textAlign:"center"}}/>
                                <span style={{color:"rgba(255,255,255,.25)",fontSize:10}}>of {competition.total_stages}</span>
                              </div>
                              <div style={{display:"flex",gap:6}}>
                                <button onClick={()=>{
                                  const raw = Number(rosterCallStage[p.id] ?? (Object.keys(p.stage_scores||{}).length+1)) || 1;
                                  setParticipantStatusManually(p,"called",raw);
                                }} disabled={p.status==="called"||p.status==="reciting"} style={{flex:1,background:"rgba(201,168,76,.12)",color:GOLD,border:`1px solid ${GOLD}44`,borderRadius:7,padding:"7px 0",cursor:(p.status==="called"||p.status==="reciting")?"not-allowed":"pointer",fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",opacity:(p.status==="called"||p.status==="reciting")?.5:1}}>Mark Called</button>
                                <button onClick={()=>setParticipantStatusManually(p,"completed")} disabled={p.status==="completed"} style={{flex:1,background:`${GREEN}18`,color:GREEN,border:`1px solid ${GREEN}44`,borderRadius:7,padding:"7px 0",cursor:p.status==="completed"?"not-allowed":"pointer",fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",opacity:p.status==="completed"?.5:1}}>Mark Done</button>
                                <button onClick={()=>setParticipantStatusManually(p,"waiting")} disabled={p.status==="waiting"} style={{flex:1,background:"rgba(255,255,255,.05)",color:"rgba(255,255,255,.6)",border:"1px solid rgba(255,255,255,.15)",borderRadius:7,padding:"7px 0",cursor:p.status==="waiting"?"not-allowed":"pointer",fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",opacity:p.status==="waiting"?.5:1}}>Back to Waiting</button>
                              </div>
                            </div>
                          )}
                        </div>
              ...
              <div style={{animation:"calledGlow 2s ease-in-out infinite",background:"rgba(201,168,76,.09)",border:`2px solid ${GOLD}`,borderRadius:14,padding:"13px"}}>
                ...
                {!pickedTile&&stageTiles.length===0&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"18px 0"}}>
                    ...
                    <button onClick={forceResync} disabled={resyncing} style={{background:`${GOLD}18`,border:`1px solid ${GOLD}44`,borderRadius:8,padding:"6px 14px",cursor:resyncing?"not-allowed":"pointer",color:GOLD,fontWeight:700,fontSize:11,fontFamily:"Cairo,sans-serif",display:"flex",alignItems:"center",gap:5}}>
                      <RefreshCw size={11} style={resyncing?{animation:"spin 1s linear infinite"}:undefined}/> {resyncing?"Syncing…":"Stuck? Tap to refresh"}
                    </button>
                  </div>
                )}
            ...
            {myParticipant.status==="reciting"&&(
              <div style={{animation:"recitingGlow 2s ease-in-out infinite",background:"rgba(34,197,94,.07)",border:`2px solid ${GREEN}`,borderRadius:14,padding:"13px"}}>
                <div style={{textAlign:"center",marginBottom:9}}>
                  ...
                  {majorCount>0&&<div style={{color:RED,fontWeight:700,fontSize:11,marginTop:2}}>🔔 {majorCount} error{majorCount!==1?"s":""}</div>}
                </div>
                {pickedTile&&(
