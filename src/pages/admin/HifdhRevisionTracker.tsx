...
// Uses admin_bulk_assign_hifdh_revision RPC (SECURITY DEFINER) for ALL assignments
// — bulk (all / level) and individual — no direct table writes needed
...
import {
  ...
  Loader2, Plus, X, Edit2, CheckCircle2, Play, Pause,
  Volume2, ArrowUpDown, Sparkles,
} from "lucide-react";

/* ── theme ── */
const G = "#1a3d24", GM = "#276749", GOLD = "#c9a84c";
const W = "#ffffff", WARM = "#faf8f4", BRD = "#e8ddd0";
...
function calcTotalPages(mode:"juz"|"hizb"|"surah", items:number[]): number {
  ...
  return items.reduce((s,n)=>s+(SURAH_PAGES[n]??2),0);
}
...
interface Assignment {
  ...
  starts_on?:string; weekend_off?:boolean; program_days?:number;
  current_position?:{item_index:number;page_offset:number};
  days_completed?:number; auto_progress?:boolean;
}
interface DailyLog {
...
/* ── form type (maps to admin_bulk_assign_hifdh_revision params) ── */
interface AForm {
  mode:"juz"|"hizb"|"surah"; selected_items:number[]; daily_pages:number;
  program_days:number; weekend_off:boolean; reciter_id:string; notes:string;
}
const defaultForm = (): AForm => ({
  mode:"juz", selected_items:[1], daily_pages:1,
  program_days:20, weekend_off:true, reciter_id:"Alafasy_128kbps", notes:"",
});
...
export default function HifdhRevisionTracker() {
  ...
  const [indForm,    setIndForm]    = useState<AForm>(defaultForm());
  ...
  const [bulkForm,   setBulkForm]   = useState<AForm>(defaultForm());
  const [bulkStep,   setBulkStep]   = useState<"config"|"confirm">("config");
  const [bulkSaving, setBulkSaving] = useState(false);
  const [bulkResult, setBulkResult] = useState<{count:number;err?:string}|null>(null);
  ...
  /* ── THE ONE RPC that handles everything ── */
  const callAssignRpc = async (
    scope:"individual"|"all"|"level",
    targetValue:string|null,
    form:AForm
  ):Promise<{count:number;err?:string}>=>{
    const {data,error}=await (supabase as any).rpc("admin_bulk_assign_hifdh_revision",{
      ...
      p_program_days:  form.program_days,
      p_weekend_off:   form.weekend_off,
      p_auto_progress: true,
  ...
  const saveIndividual=async(sid:string)=>{
    ...
    const res=await callAssignRpc("individual",sid,indForm);
    if(res.err){setSaveErr(`Failed: ${res.err}`);}
    else{setShowAssign(null);await load();}
    setSaving(false);
  };
  ...
  const saveBulk=async()=>{
    setBulkSaving(true);
    let res:{count:number;err?:string};
    if(bulkTarget==="all"){
      res=await callAssignRpc("all",null,bulkForm);
    } else {
      res=await callAssignRpc("level",bulkLevel,bulkForm);
    }
    setBulkResult(res);
    await load();
    setBulkSaving(false);
  ...
  const bulkCount=bulkTarget==="all"?students.length:(lvCounts[bulkLevel]||0);
    ...
    <div style={{background:WARM,minHeight:"100%",fontFamily:"system-ui,sans-serif"}}
      ...
      <div style={{background:G,padding:"16px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <button onClick={e=>{e.stopPropagation();setShowBulk(true);setBulkStep("config");setBulkResult(null);setBulkForm(defaultForm());}}
      ...
      <div style={{background:W,borderBottom:`1px solid ${BRD}`,padding:"10px 14px"}}>
        ...
        <button onClick={e=>{e.stopPropagation();setShowBulk(true);setBulkStep("config");setBulkResult(null);setBulkForm(defaultForm());}}
          ...
          <span style={{fontSize:10,fontWeight:500,opacity:0.8}}>— all / by level</span>
        </button>
        {/* Search */}
            ...
            <div style={{flex:1,overflowY:"auto",padding:"16px 18px 28px"}}>
              {bulkResult!==null?(
                ...
                <div style={{textAlign:"center",padding:"32px 16px"}}>
                  <div style={{fontSize:52,marginBottom:12}}>{bulkResult.err?"⚠️":"✅"}</div>
                  <div style={{fontWeight:800,fontSize:20,color:G,marginBottom:8}}>
                    {bulkResult.err?`Failed`:`Assigned to ${bulkResult.count} students!`}
                  </div>
                  {bulkResult.err&&(
                    <div style={{padding:"10px 14px",borderRadius:12,background:"#fff5f5",
                      border:"1px solid #fca5a5",fontSize:12,color:"#dc2626",marginBottom:16,textAlign:"left"}}>
                      {bulkResult.err}
                    </div>
                  )}
                  <button onClick={()=>setShowBulk(false)}
                ...
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  <div style={{padding:"14px 16px",borderRadius:14,background:`${G}0d`,border:`1px solid ${G}20`}}>
                    ...
                    {[
                      ["Target", bulkTarget==="all"?`All ${students.length} students`:`${bulkCount} students — ${LV_LABEL[bulkLevel]||bulkLevel}`],
                      ...
                      ["Duration",`${bulkForm.program_days} active days`],
                      ["Weekends",bulkForm.weekend_off?"Sunday off":"Every day"],
                    ].map(([k,v])=>(
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <button onClick={saveBulk} disabled={bulkSaving}
                      ...
                      {bulkSaving?"Assigning…":`✅ Assign to ${bulkCount} Students`}
                    </button>
                  </div>
                </div>
              ):(
                ...
                <div style={{display:"flex",flexDirection:"column",gap:16}}>
                  {/* Target picker */}
                  <div>
                    <div style={{fontSize:10,fontWeight:800,color:"#9aab94",marginBottom:8}}>ASSIGN TO</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                      {([
                        {k:"all",  icon:"👥",title:"All Students",  desc:`${students.length} students`},
                        {k:"level",icon:"🎓",title:"By Level",      desc:"Choose a level"},
                    ...
                    {bulkTarget==="level"&&(
                      <div style={{display:"flex",flexDirection:"column",gap:6}}>
                        {LEVELS.map(lv=>(
                          <button key={lv} onClick={()=>setBulkLevel(lv)}
                            style={{padding:"10px 14px",borderRadius:12,cursor:"pointer",
                              display:"flex",alignItems:"center",gap:10,
                              border:`2px solid ${bulkLevel===lv?LV_COLOR[lv]:BRD}`,
                              background:bulkLevel===lv?`${LV_COLOR[lv]}10`:W}}>
                            <div style={{width:10,height:10,borderRadius:"50%",
                              background:bulkLevel===lv?LV_COLOR[lv]:BRD}}/>
                            <span style={{flex:1,textAlign:"left",fontSize:13,fontWeight:700,
                              color:bulkLevel===lv?LV_COLOR[lv]:"#374151"}}>{LV_LABEL[lv]}</span>
                            <span style={{fontSize:11,color:"#9aab94"}}>{lvCounts[lv]||0} students</span>
                            {bulkLevel===lv&&<span>✓</span>}
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                  <ProgramForm form={bulkForm} setForm={setBulkForm}/>
                  <button
                    onClick={()=>setBulkStep("confirm")}
                    disabled={bulkForm.selected_items.length===0||bulkCount===0||(bulkTarget==="level"&&!bulkLevel)}
                    style={{padding:14,borderRadius:12,border:"none",
                      background:`linear-gradient(135deg,${G},${GM})`,
                      color:W,fontWeight:800,fontSize:14,cursor:"pointer",
                      opacity:bulkForm.selected_items.length===0||bulkCount===0||(bulkTarget==="level"&&!bulkLevel)?0.4:1}}>
                    Review for {bulkCount} students →
                  </button>
                </div>
              )}
            </div>
  ...
  function StudentRow({s}:{s:Student}){
    const log=s.todayLog, assign=s.assignment;
    const isDone=!!log?.completed, isAcked=!!log?.acknowledged_at;
    const isOpen=expanded===s.user_id;
    // progress from current_position tracked by daily cron
    const cp=assign?.current_position;
    const daysLeft=assign?(assign.program_days||30)-(assign.days_completed||0):null;
    const pct=assign?Math.min(100,Math.round(((assign.days_completed||0)/(assign.program_days||30))*100)):null;
        ...
        <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontWeight:700,fontSize:13,color:"#111827",
              whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{s.full_name}</div>
            <div style={{fontSize:10,color:"#9aab94",marginTop:1,display:"flex",gap:5,alignItems:"center"}}>
              ...
              {s.level&&(
                <span style={{background:`${LV_COLOR[s.level]||"#888"}18`,
                  color:LV_COLOR[s.level]||"#888",padding:"1px 6px",borderRadius:6,fontWeight:700,fontSize:9}}>
          ...
          <div style={{borderTop:`1px solid ${BRD}`,padding:"12px 14px",
            ...
            {assign&&pct!==null&&(
              <div style={{background:W,border:`1px solid ${BRD}`,borderRadius:12,padding:"10px 12px"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6,fontSize:11}}>
                  ...
                  <span style={{color:"#9aab94"}}>{assign.days_completed||0} / {assign.program_days||30} days</span>
                </div>
                ...
                <div style={{textAlign:"right",fontSize:10,color:"#9aab94",marginTop:3}}>{pct}% complete</div>
              </div>
            )}

            {/* Assignment info + edit */}
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
              ...
              <button onClick={()=>{
                ...
                setIndForm({
                  ...
                  daily_pages:assign?.daily_pages??1, program_days:assign?.program_days??20,
                  weekend_off:assign?.weekend_off??true, reciter_id:assign?.reciter_id??"Alafasy_128kbps",
                  notes:assign?.notes??"",
                });
                setSaveErr(null);
              }}
            ...
            {showAssign===s.user_id&&(
              <div style={{background:W,border:`1px solid ${BRD}`,borderRadius:14,padding:14}}>
                ...
                <ProgramForm form={indForm} setForm={setIndForm}/>
                {saveErr&&(
...
/* ════════════════════════════════════════════
   ProgramForm — shared form for assign config
   ════════════════════════════════════════════ */
function ProgramForm({form,setForm}:{form:AForm;setForm:(f:AForm|((p:AForm)=>AForm))=>void}){
  const set=(patch:Partial<AForm>)=>setForm(f=>({...f,...patch}));
  ...
  useEffect(()=>{
    if(autoDays!==null) setForm(f=>({...f,program_days:autoDays}));
  },[autoDays]);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{background:`${G}08`,border:`1.5px solid ${G}20`,borderRadius:12,padding:"12px 14px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10,fontSize:12,flexWrap:"wrap"}}>
          <span style={{background:W,border:`1px solid ${BRD}`,borderRadius:8,padding:"4px 10px",fontWeight:700,color:G}}>
            {form.selected_items.length} {form.mode==="juz"?"Juz":form.mode==="hizb"?"Hizb":"Surah"}
          </span>
          <span style={{color:"#9aab94"}}>×</span>
          <span style={{background:W,border:`1px solid ${BRD}`,borderRadius:8,padding:"4px 10px",fontWeight:700,color:G}}>
            {form.mode==="juz"?"20p":form.mode==="hizb"?"10p":`~${totalPages}p`}
          </span>
          <span style={{color:"#9aab94"}}>÷</span>
          <span style={{background:W,border:`1px solid ${BRD}`,borderRadius:8,padding:"4px 10px",fontWeight:700,color:G}}>
            {form.daily_pages===0.5?"½":form.daily_pages}/day
          </span>
          <span style={{color:"#9aab94"}}>=</span>
          <span style={{fontWeight:900,fontSize:18,color:G}}>{form.program_days} days</span>
        </div>
        <div style={{background:W,border:`1px solid ${BRD}`,borderRadius:10,
          padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:22,fontWeight:900,color:G}}>{form.program_days} days</div>
            <div style={{fontSize:10,color:"#9aab94"}}>
              {totalPages} total pages · ~{Math.ceil(form.program_days/30)} month{form.program_days>30?"s":""}
            </div>
          </div>
          <div style={{fontSize:28}}>{form.program_days<=30?"📅":form.program_days<=60?"📆":"🗓️"}</div>
        </div>
        <div style={{marginTop:8,display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:10,color:"#9aab94"}}>Override:</span>
          <input type="number" min={1} value={form.program_days}
            onChange={e=>set({program_days:parseInt(e.target.value)||1})}
            style={{width:70,padding:"5px 8px",borderRadius:8,border:`1px solid ${BRD}`,
              fontSize:12,color:G,background:WARM,textAlign:"center"}}/>
          <span style={{fontSize:10,color:"#9aab94"}}>days</span>
        </div>
      </div>

      {/* Weekends */}
      <div>
        <div style={{fontSize:10,fontWeight:800,color:"#9aab94",marginBottom:6}}>WEEKEND REST</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[
            {v:true, label:"Sunday off",  icon:"🌙", desc:"6 days/week"},
            {v:false,label:"Every day",   icon:"📖", desc:"7 days/week"},
          ].map(opt=>(
            <button key={String(opt.v)} onClick={()=>set({weekend_off:opt.v})}
              style={{padding:"10px 12px",borderRadius:12,cursor:"pointer",textAlign:"left",
                border:`2px solid ${form.weekend_off===opt.v?G:BRD}`,
                background:form.weekend_off===opt.v?`${G}0d`:W}}>
              <div style={{fontSize:18}}>{opt.icon}</div>
              <div style={{fontSize:12,fontWeight:800,color:G,marginTop:4}}>{opt.label}</div>
              <div style={{fontSize:10,color:"#9aab94"}}>{opt.desc}</div>
            </button>
          ))}
        </div>
