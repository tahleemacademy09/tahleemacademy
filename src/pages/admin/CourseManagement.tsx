...
// Full-height image for horizontal card layout
const SubjThumb = ({ url, title, bg }: { url?: string | null; title: string; bg: string }) => {
  const [src, setSrc] = useState<string | null>(null);
  const [err, setErr] = useState(false);
  ...
  if (!src || err) {
    return (
      <div style={{ width: "100%", height: "100%", background: bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <BookOpen size={22} style={{ opacity: 0.25 }} />
      </div>
    );
  }
  return (
      ...
      {unlinked.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: 16 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {unlinked.map((s: any) => (
              <button key={s.id} onClick={async () => { await supabase.from("subjects").update({ course_id: selCourse?.id } as any).eq("id", s.id); qc.invalidateQueries({ queryKey: ["adm-subjects"] }); qc.invalidateQueries({ queryKey: ["adm-all-subjects"] }); }}
...
export default function CourseManagement() {
  ...
  const { data: privateStudents = [] } = useQuery({ queryKey: ["private-students-list"], queryFn: async () => { const { data } = await (supabase as any).from("profiles").select("user_id, full_name, student_id").eq("student_type", "private"); return data || []; } });
