...
import {
  ...
  Zap, ThumbsUp, ThumbsDown, Minus, RotateCcw, Keyboard, ChevronDown
} from "lucide-react";
          ...
          {q && (
            <div style={{ maxWidth: 720, margin: "0 auto", width: "100%", animation: "slideIn .2s ease" }} key={currentIdx}>
              <div style={{ background: "#fff", borderRadius: 20, boxShadow: "0 4px 24px rgba(0,0,0,.1)", overflow: "hidden" }}>
                ...
                {/* Passage / guidelines — pinned above every question when the exam has one,
                    not just shown once pre-exam, since comprehension questions need it visible
                    while answering. Collapsible so it doesn't dominate the screen on repeat views. */}
                {(exam?.guidelines || exam?.guidelines_ar) && q?.metadata?.show_passage && (
                  <div style={{ borderBottom: `1px solid ${BORDER}` }}>
                    <button onClick={() => setPassageOpen(o => !o)}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "12px 22px", background: "#f8fafb", border: "none", cursor: "pointer", textAlign: "left" as const }}>
                      <BookOpen style={{ width: 15, height: 15, color: GOLD, flexShrink: 0 }} />
                      <span style={{ fontSize: 11, fontWeight: 800, color: G, letterSpacing: .5, flex: 1 }}>{t("PASSAGE", "النَّصّ")}</span>
                      <ChevronDown style={{ width: 16, height: 16, color: "#7a9e88", transform: passageOpen ? "rotate(180deg)" : "none", transition: "transform .15s" }} />
                    </button>
                    {passageOpen && (
                      <div style={{ padding: "4px 22px 18px", maxHeight: 320, overflow: "auto" }}>
                        <div style={{ fontSize: 14.5, color: "#374151", lineHeight: 1.9, fontFamily: "'Amiri',serif", whiteSpace: "pre-wrap" }} dir="auto">
                          {language === "ar" ? (exam.guidelines_ar || exam.guidelines) : (exam.guidelines || exam.guidelines_ar)}
                        </div>
                      </div>
                    )}
                  </div>
