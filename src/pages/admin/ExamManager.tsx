...
export default function ExamManager() {
  ...
  const duplicateExam = async (exam: any) => {
    ...
    const { data: ne } = await supabase.from("exams").insert({
      ...exam, id: undefined, title: exam.title + " (Copy)", is_published: false, created_at: undefined, updated_at: undefined,
    }).select().single();
    if (ne && qs?.length) {
      await supabase.from("exam_questions").insert(qs.map((q: any) => ({ ...q, id: undefined, exam_id: ne.id })));
    }
  ...
  const doAssign = async () => {
    ...
    try {
      ...
      // Get already-assigned students (avoid duplicates)
      ...
      if (newIds.length > 0) {
        // FIX: Insert assignments
        const { error: aErr } = await supabase.from("exam_assignments").insert(
          newIds.map(uid => ({ exam_id: assignExam.id, user_id: uid, assigned_by: user?.id }))
        );
        if (aErr) throw aErr;

        // FIX: Send in-app notifications so StudentExams shows the exam immediately
        const { error: nErr } = await supabase.from("notifications" as any).insert(
          newIds.map(uid => ({
            user_id: uid,
            title: `📝 New exam assigned: ${assignExam.title}`,
            message: `You have been assigned the exam "${assignExam.title}". ${
              assignExam.start_date ? `Available from: ${new Date(assignExam.start_date).toLocaleDateString()}` : "You can take it now."
            }`,
            type: "exam_assigned",
            reference_id: assignExam.id,
            is_read: false,
          }))
        );
        if (nErr) console.warn("Notification insert warning:", nErr.message);
      }

      toast({
        title: `✅ Assigned to ${newIds.length} student${newIds.length !== 1 ? "s" : ""}`,
        description: existingSet.size > 0 ? `${existingSet.size} were already assigned` : undefined,
      });
      setAssignExam(null);
      fetchExams();
    } catch (e: any) {
