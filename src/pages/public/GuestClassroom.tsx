...
import { LogOut, UserPlus, Radio, Users } from "lucide-react";
...
const GuestClassroom = () => {
  ...
  const { token, url, room, guestName, classTitle, classTitleAr, isHost } = (location.state || {}) as {
    token?: string;
    url?: string;
    room?: string;
    guestName?: string;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "#0f3122", color: "white", fontFamily: "'Cairo', sans-serif" }}>
        <div className="max-w-md w-full text-center">
          ...
          {isHost ? (
            <Link to="/admin/public-classes" className="text-sm text-white/40 hover:text-white/60 underline">
              Back to Dashboard
            </Link>
          ) : (
