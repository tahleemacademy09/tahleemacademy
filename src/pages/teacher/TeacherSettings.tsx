...
export default function TeacherSettings() {
  ...
  const loadTeacherPayments = async () => {
    ...
    try {
      ...
      if (error) console.error("teacher_payments fetch error:", error.message);
  ...
  const verifyAccount = async () => {
    ...
    try {
      ...
      // supabase-js v2: FunctionsHttpError.context is a Response object (not a string).
      // Must await .text() on it before parsing.
      if (error) {
