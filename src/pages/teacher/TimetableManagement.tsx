...
export default function TimetableManagement() {
  ...
  const { data: slots, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // NOTE: subject_timetable has no FK to profiles for teacher_id (only
      // subject_id -> subjects), so `profiles!subject_timetable_teacher_id_fkey`
      // was never a resolvable embed — this query errored on every load and
      // silently fell back to the no-teacher-names query below it. Fetch
      // profiles separately (covering both the primary teacher_id and any
      // co-teachers in teacher_ids[]) and merge instead.
      ...
      const teacherIds = [...new Set(
        data.flatMap((s: any) => [s.teacher_id, ...(Array.isArray(s.teacher_ids) ? s.teacher_ids : [])]).filter(Boolean)
      )];
      ...
      if (teacherIds.length) {
        ...
        teachersById = Object.fromEntries((teachers || []).map((t: any) => [t.user_id, t]));
      }
      const withTeachers = data.map((s: any) => ({ ...s, teacher: teachersById[s.teacher_id] || null }));

      // Fetch private student counts per slot
      const ids = withTeachers.map((s: any) => s.id);
      if (ids.length > 0) {
        ...
        return withTeachers.map((s: any) => ({ ...s, _privCount: countMap[s.id] || 0 }));
      }
      return withTeachers;
    },
  });
