...
const TeacherClasses = () => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between"><Label>{t("Record?", "تسجيل؟")}</Label><Switch checked={form.is_recorded} onCheckedChange={v => setForm({ ...form, is_recorded: v })} /></div>
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5" />{t("Upcoming Classes", "الحصص القادمة")}</CardTitle></CardHeader>
            ...
            <div key={s.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {s.status === "active" && <Badge className="bg-green-500 text-white">LIVE</Badge>}
                </div>
              ...
              <Button size="sm" onClick={() => navigate(`/student/subjects/${s.subject_id}`)}>{t("Open", "فتح")}</Button>
            </div>
          ))}
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5" />{t("Past Classes", "الحصص السابقة")}</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          {past.slice(0, 20).map(s => (
            <div key={s.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {(s as any).subjects?.title} • {s.scheduled_at ? format(new Date(s.scheduled_at), "MMM d, yyyy") : ""} • {s.total_participants || 0} {t("participants", "مشاركين")}
                </p>
              </div>
