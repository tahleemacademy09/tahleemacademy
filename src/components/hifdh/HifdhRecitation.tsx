...
// STATIC LAYOUT + MULTI-VERSE HIGHLIGHTING + SMART COUNTING
//   • Header & Footer fixed — only Quran text scrolls
//   • Mobile-optimized with 100dvh height
//   • Highlights ALL active verses when reading pairs/cumulative
//   • Speech recognition combines multiple verses — counts ONLY when full text matched
//   • Visual feedback shows which verses are being recited
...
const SESSION_KEY = "hifdh_mem_v6";
...
function normalizeHurufMuqattaat(text: string): string {
  return text
    // Multi-letter combinations first (longest match wins)
    ...
    // Single-letter names (only when standalone — avoid mangling regular words)
    .replace(/\bصاد\b/g,  "ص")
    .replace(/\bقاف\b/g,  "ق")
    .replace(/\bنون\b/g,  "ن");
}
...
/* ── Combined text builder for multi-verse matching ──────────── */
function buildCombinedText(targets: Ayah[]): string {
  return targets.map(a => a.text).join(" ");
}

function matchScore(transcript: string, expectedText: string): number {
  ...
  if (!eWords.length) return 0;
    const matched = eWords.filter(ew => tWords.some(tw => tw.includes(ew) || ew.includes(tw))).length;
  const ratio = matched / eWords.length;
  
  // For combined verses, require higher confidence
  if (ratio < 0.6 && e.includes(t) && t.length > e.length * 0.5) return 0.7;
  return ratio;
}
...
/* ══════════════════════════════════════════════════════════════
   COMPONENT - STATIC HEADER/FOOTER + MULTI-VERSE HIGHLIGHT
══════════════════════════════════════════════════════════════ */
export default function HifdhMemorization({ reciter: reciterProp }: Props) {
  ...
  const [selectedReciter, setSelectedReciter] = useState(reciterProp || DEFAULT_RECITER);  const [peeking,         setPeeking]         = useState(false);
  ...
  /* ── Web Speech Recognition with Multi-Verse Support ───────── */
  ...
  const startSR = useCallback(() => {
    ...
    setMicError("");
    lastCountedText.current = "";    cooldownRef.current     = false;
    ...
    sr.onresult = (event: any) => {
      ...
      // Build combined expected text for multi-verse matching
      ...
      // Update highlighting based on partial match
      const transcriptNorm = norm(bestTranscript);
      const activeVerses = new Set<number>();
      
      targets.forEach((ayah, idx) => {
        const ayahNorm = norm(ayah.text);
        // Check if this specific verse appears in transcript
      ...
      // Only count on FINAL result with high confidence match to COMBINED text
      if (hasFinal) {
        const score = matchScore(bestTranscript, combinedText);
        const normText = norm(bestTranscript);
        
        // Require 60%+ match for multi-verse, 45% for single
        const threshold = targets.length > 1 ? 0.6 : 0.45;
        
        if (score >= threshold && normText !== lastCountedText.current && normText.length > 5) {
  ...
  const advanceStep = useCallback(() => {
    ...
    if (idx < all.length - 1) {
      ...
      setStepIdx(next); setRepsDone(0); setPeeking(false); setRecitingVerses(new Set());
      stopAudio();
  ...
  useEffect(() => {
    ...
    if (activeIndices.length > 0 && sessionAyahsRef.current.length > 0) {
      const firstActiveVerse = sessionAyahsRef.current[activeIndices[0]]?.numberInSurah;      if (firstActiveVerse && verseRefs.current[firstActiveVerse]) {
  ...
  const startSession = () => {
    ...
    const ns = buildSteps(slice.length, repsPerVerse);    stepsRef.current        = ns;
      ...
      <div style={{ height: "100%", display: "flex", flexDirection: "column", background: LIGHT, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: "#7a9e88" }}>
                  {srAvailable ? "Multi-verse recognition active" : "Use Chrome on Android"}
                </div>
              </div>
          ...
          <div style={card({ padding: "10px" })}>
            ...
            </select>            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              {([
          ...
          <div style={card({ padding: "8px 10px" })}>
            ...
            <div style={{ display: "flex", gap: 4, overflowX: "auto", paddingBottom: 2, scrollbarWidth: "none" }}>              {RECITERS.map(r => (
  ...
  /* ─────────────────────────────────────────────────────────────
     ACTIVE SESSION - STATIC HEADER/FOOTER + SCROLLABLE QURAN
  ───────────────────────────────────────────────────────────── */
  ...
  const col        = STEP_STYLE[currentStep.type];  const progress   = steps.length > 1 ? (stepIdx / (steps.length - 1)) * 100 : 100;
  const pool       = sessionAyahsRef.current;
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#fff", overflow: "hidden" }}>
      <style>{`
        ...
        @keyframes highlightPulse{0%,100%{box-shadow:0 0 0 0 rgba(183,121,31,0.4)}50%{box-shadow:0 0 0 8px rgba(183,121,31,0)}}
      ...
      {/* ── HEADER - STATIC (Fixed) ──────────────────────────── */}
      <div style={{ background: "#fff", borderBottom: `1px solid ${BORDER}`, padding: "8px 12px", flexShrink: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ overflow: "hidden" }}>
              ...
              <div style={{ fontFamily: "'Amiri',serif", fontSize: 10, color: col.text, opacity: .8, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {currentStep.labelAr}
              </div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            </span>            <button onClick={() => { stopAudio(); stopSR(); clearSession(); setStarted(false); setStepIdx(0); setRepsDone(0); }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ overflow: "hidden" }}>
            ...
            <div style={{ fontFamily: "'Amiri',serif", fontSize: 10, color: "rgba(255,255,255,.7)", direction: "rtl", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
        ...
        <div style={{ padding: "12px", display: "flex", flexDirection: "column", gap: 8 }}>
          {pool.map((ayah, poolIdx) => {
            const isActive = activeSet.has(poolIdx);            const isReciting = recitingVerses.has(ayah.numberInSurah);
            ...
            return (
              <div 
                ...
                style={{
                  ...
                  border:      isActive
                    ? `2px solid ${currentStep.type === "single" ? GOLD : BORDER}`
                    : "1px solid rgba(200,190,170,0.3)",
                  borderRadius: 12,
                  padding:      isActive ? "12px 14px" : "6px 14px",
                  opacity:      isHidden ? 0.4 : 1,
                  filter:       isHidden ? "blur(2px)" : "none",
                  boxShadow:    isActive ? (isReciting ? "0 4px 12px rgba(183,121,31,.2)" : "0 2px 8px rgba(26,61,36,.1)") : "none",
                  transition:   "all .3s ease",
                  pointerEvents: isHidden ? "none" : "auto",
                  ...(isReciting ? { borderLeft: `4px solid ${GOLD}` } : {}),
                }}>
                ...
                {isActive && (
                  <>
                    ...
                    <div style={{
                      ...
                      textAlign: "right",
                      direction: "rtl",
                      opacity:    hideText ? 0.08 : 1,                      filter:     hideText ? "blur(10px)" : "none",
                      transition: "opacity .2s, filter .2s",
                      wordBreak: "break-word",
                      ...(isReciting ? { className: "verse-reciting" } : {}),
                    }} className={isReciting ? "verse-reciting" : ""}>
                      <span className={isReciting ? "verse-highlight" : ""}>
                        {ayah.text}
                      </span>
                    </div>
                    ...
                    {isCumul && (
                      <button className="peek-btn"
                        ...
                        style={{ width: "100%", padding: "6px 0", borderRadius: 8, marginTop: 8,
                ...
                {isHidden && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, direction: "ltr" }}>
                    <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#d8cfc0",
                      display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontFamily: "'Amiri',serif", fontSize: 10, color: "#888" }}>
                        {toAr(ayah.numberInSurah)}
                      </span>
                    </div>
                    <div style={{ flex: 1, height: 6, borderRadius: 4, background: "rgba(180,165,140,0.3)" }} />
                  </div>
                )}
              </div>
            );
        ...
        {isGoodText && (
          <div style={{
            ...
            background: "#f0fff4", border: `1px solid ${BORDER}`,            fontSize: 16, direction: "rtl", fontFamily: "'Amiri',serif", color: G,
        ...
        {isMultiVerse && micActive && (
          <div style={{
            ...
            🔗 Reciting {currentStep.indices.length} verses together — wait for complete match
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#fff", borderTop: `1px solid ${BORDER}`, flexShrink: 0 }}>
        ...
        {!isOverview && (
          <div style={{
            ...
            {/* Rep dots */}
            <div style={{ display: "flex", gap: 5, flex: 1, overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
            ...
            {micActive && (
              <div style={{ flexShrink: 0, fontSize: 11, fontWeight: 700, color: "#ef4444", minWidth: 36, textAlign: "center" }}>
        ...
        {/* Mic error */}
        {micError && (
          <div style={{ margin: "6px 12px 0", padding: "6px 10px", borderRadius: 8, background: "#fffbeb",
        ...
        <div style={{ display: "flex", gap: 8, padding: "10px 12px" }}>
          {!isOverview && (
            micActive ? (
              <button onClick={stopSR}                style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "none",
          ...
          <button onClick={markRep}
            style={{ flex: isOverview ? 2 : 1, padding: "12px 0", borderRadius: 10, border: "none",
        ...
        <button
          onClick={() => {
            stopAudio(); stopSR(); setPeeking(false); setRecitingVerses(new Set());
            if (stepIdx > 0) {
          ...
          style={{ margin: "0 12px 12px", padding: "10px 0", borderRadius: 8, border: `1px solid ${BORDER}`,
