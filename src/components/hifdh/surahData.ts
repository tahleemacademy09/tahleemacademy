...
// Complete surah list — SURAHS array is used by all Hifdh components
...
// audioUrl(surahNum, ayahNum) — used by Memorization, Recitation, Exercise, Test
export const audioUrl = (surahNum: number, ayahNum: number): string => {
  const s = String(surahNum).padStart(3, "0");
  const a = String(ayahNum).padStart(3, "0");
  // Primary CDN: EveryAyah (reliable, per-ayah)
  return `https://cdn.islamic.network/quran/audio/128/ar.alafasy/${(surahNum - 1) * 1000 + ayahNum}.mp3`;
};

// Fallback: surah-level audio (used if per-ayah fails)
