...
const ClassroomView=({subject,onLeave,onMinimize,autoJoin=false}:ClassroomViewProps)=>{
  ...
  useEffect(()=>{
    ...
    supabase.from("subject_materials" as any).select("file_url,material_type").eq("subject_id",subject.id)
      ...
      },()=>{}); // best-effort — Materials panel will still load normally if this fails
        ...
        <LiveKitRoom key={roomKey} serverUrl={wsUrl} token={token} connect={phase==="live"} audio={false} video={false} options={{
            ...
            publishDefaults:{
              ...
              videoSimulcastLayers:[
                {resolution:{width:320,height:180,frameRate:15}, encoding:{maxBitrate:150_000, maxFramerate:15}},
                {resolution:{width:640,height:360,frameRate:24}, encoding:{maxBitrate:500_000, maxFramerate:24}},
                {resolution:{width:1280,height:720,frameRate:30}, encoding:{maxBitrate:1_500_000,maxFramerate:30}},
              ],
            },
