  ...
  useEffect(()=>{
    ...
    const applyAdaptiveVideo=async(q:string)=>{
      ...
      try{
        ...
        if(q==="poor"){
          // Drop to 240p / 10fps to ease bandwidth pressure
          ...
          await lp.setCameraEnabled(false);
          await lp.setCameraEnabled(true,{
            resolution:{width:240,height:180,frameRate:8},
          } as any);
        }else if(q==="good"||q==="excellent"){
          // Restore normal quality when network recovers
          if(lastQualityRef.current==="poor"||lastQualityRef.current==="lost"){
            await lp.setCameraEnabled(false);
            await lp.setCameraEnabled(true,{
      ...
      }catch{/* camera may be off — safe to ignore */}
    };
    ...
    const onQualityChanged=(_participant:any,q:ConnectionQuality)=>{
      ...
      // Toast warning — throttled to once every 30s to avoid spam
      const now=Date.now();
      if((label==="poor"||label==="lost")&&now-lastToastRef.current>30_000){
        lastToastRef.current=now;
        toast({
          title: label==="lost"?"⚠️ Very Poor Network":"⚠️ Poor Network",
          description: label==="lost"
            ?"Your connection is very weak. Video quality has been reduced to stay connected."
            :"Network quality is degrading. Video quality reduced to prevent disconnection.",
        });
      }
    ...
    // ConnectionQualityChanged fires for any participant — filter to local only
...
const NetworkQualityBadge=()=>{
  ...
  const color = quality==="excellent"||quality==="good" ? "#34d399"
              ...
              : quality==="lost"                         ? "#ef4444"
              :                                            "rgba(255,255,255,.4)";
  const bg    = quality==="excellent"||quality==="good" ? "rgba(52,211,153,.1)"
              : quality==="poor"                         ? "rgba(251,191,36,.1)"
              : quality==="lost"                         ? "rgba(239,68,68,.12)"
              :                                            "rgba(255,255,255,.06)";
  const bdr   = quality==="excellent"||quality==="good" ? "rgba(52,211,153,.25)"
              : quality==="poor"                         ? "rgba(251,191,36,.3)"
              : quality==="lost"                         ? "rgba(239,68,68,.3)"
              :                                            "rgba(255,255,255,.1)";
  return(
    <div title={`Network: ${QUALITY_LABELS[quality]||"Unknown"}`} style={{
      display:"flex",alignItems:"center",gap:5,
      background:bg,border:`1px solid ${bdr}`,
      borderRadius:20,padding:"3px 8px",flexShrink:0,cursor:"default",
    }}>
      <SignalBars quality={quality}/>
      <span style={{fontSize:11,fontWeight:600,color,fontFamily:"'Google Sans',sans-serif",whiteSpace:"nowrap"}}>
        ...
        <LiveKitRoom key={roomKey} serverUrl={wsUrl} token={token} connect={phase==="live"} audio={false} video={false} options={{adaptiveStream:{pixelDensity:"screen"},dynacast:true,disconnectOnPageLeave:false,audioCaptureDefaults:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,sampleRate:48000,channelCount:1},publishDefaults:{audioPreset:{maxBitrate:64000},dtx:false,red:true,stopMicTrackOnMute:false,videoEncoding:{maxBitrate:700_000,maxFramerate:20},backupCodec:true},videoCaptureDefaults:{resolution:{width:640,height:480,frameRate:20},facingMode:"user"}}} style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,position:"relative"}} data-lk-theme="default">
          ...
          <div style={{
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
              ...
              {/* Network quality — always visible so users know why calls drop */}
              <NetworkQualityBadge/>
              {/* Participant count */}
