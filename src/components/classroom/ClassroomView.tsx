...
const RoomDataListener = ({ onWbOpen, onWbClose, strokesBuffer, onMatOpen, onMatClose, onWbAllowWrite, onRecAllowed, onEmojiReact, onGroupRecite, onHandRaise, onAdminMuteAll, onClassEnded }: any) => {
  ...
  useEffect(() => {
    ...
    room.on(RoomEvent.DataReceived, h);
    return () => { room.off(RoomEvent.DataReceived, h); delete (window as any).lkRoom; };
  }, [room]);
  return null;
};
...
const Whiteboard = ({ room, onClose, isTeacher, initialStrokes, subjectId, canStudentWrite }: any) => {
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "#f8f8f8", display: "flex", flexDirection: "column" }}>
      <div style={{ background: `linear-gradient(135deg,${TEAL2},${TEAL})`, display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", flexShrink: 0, boxShadow: "0 2px 16px rgba(0,0,0,.4)", overflowX: "auto" }}>
...
/* ═ IN-CLASS MATERIAL VIEWER ══ */const InClassMaterialViewer = ({ material, onClose, isTeacher = false }: any) => {
  ...
  const pdfSrc = kind === "pdf" && resume?.page ? `${embedUrl}#page=${resume.page}` : embedUrl;
  ...
  const navigatePdfPage = (p: number) => {
    ...
    if (iframeRef.current) { iframeRef.current.src = `${embedUrl}#page=${p}`; }
  };

  const overlayStyle: React.CSSProperties = fullscreen ? { position: "fixed", inset: 0, zIndex: 9990 } : { position: "absolute", inset: 0, zIndex: 60 };
  ...
  return (
    <div style={{ ...overlayStyle, background: "#0f1117", display: "flex", flexDirection: "column", animation: "fade-in .18s ease" }}>
      <div style={{ height: 46, background: "rgba(6,78,59,.97)", display: "flex", alignItems: "center", padding: "0 10px", gap: 8, flexShrink: 0, borderBottom: "1px solid rgba(255,255,255,.08)" }}>
        ...
        <button onClick={() => setFullscreen(v => !v)} title={fullscreen ? "Exit fullscreen" : "Fullscreen"} style={{ width: 30, height: 30, borderRadius: 8, background: "rgba(255,255,255,.1)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{fullscreen ? <Minimize2 style={{ width: 13, height: 13 }} /> : <Maximize2 style={{ width: 13, height: 13 }} />}</button>
        <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, background: "rgba(255,255,255,.12)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><X style={{ width: 13, height: 13 }} /></button>
      </div>
      {renderContent()}
    </div>
  );
...
const SubjectMaterialsPanel = ({ subjectId, subject, onClose }: any) => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 55, background: "rgba(0,0,0,.55)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "min(360px,100%)", background: "#13181f", borderLeft: "1px solid rgba(255,255,255,.08)", display: "flex", flexDirection: "column", animation: "slide-up .2s ease", boxShadow: "-8px 0 32px rgba(0,0,0,.5)" }}>
            ...
            return (
              <button key={m.id} onClick={() => setViewing(m)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 12, cursor: "pointer", textAlign: "left", marginBottom: 8, transition: "background .12s" }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,.09)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,.04)")}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ margin: "3px 0 0", fontSize: 11, color: "rgba(255,255,255,.35)", textTransform: "capitalize" }}>{m.material_type || "file"}{resume?.time && <span style={{ marginLeft: 6, color: TEAL }}>▶ {Math.floor((resume.time || 0) / 60)}m</span>}{resume?.page && !resume?.time && <span style={{ marginLeft: 6, color: TEAL }}>p.{resume.page}</span>}</p>
                </div>
  ...
  const startRec = async () => {
    try {
      ...
      if (!audio) {
        ...
        try { audio = await navigator.mediaDevices.getUserMedia({ audio: true, video: false }); } catch (gumErr: any) {
          toast({ title: "Microphone access denied", description: gumErr?.message || "Check browser permissions", variant: "destructive" }); return;
        }
      }
      chunksRef.current = [];
  ...
  const stopRec = async () => {
    ...
    mr.onstop = async () => {
      try {
        ...
        if (blob.size === 0) { toast({ title: "Recording empty", description: "No audio was captured. Make sure your mic is on.", variant: "destructive" }); return; }
        ...
        if (upErr) { console.error("[RecController] upload error: ", upErr); throw new Error(upErr.message); }
        ...
        acRef.current?.close(); acRef.current = null; chunksRef.current = []; onSavingChange?.(false);
      }
    };
...
const RoomSettingsModal = ({ onClose, room }: { onClose: () => void; room: any }) => {
  ...
  const DeviceRow = ({ device, selected, onClick }: { device: MediaDeviceInfo; selected: boolean; onClick: () => void }) => (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 10, border: "1px solid", borderColor: selected ? "#22c55e" : "rgba(255,255,255,.1)", background: selected ? "rgba(34,197,94,.12)" : "rgba(255,255,255,.03)", cursor: "pointer", width: "100%", marginBottom: 6 }}>
      <div style={{ width: 16, height: 16, borderRadius: "50%", flexShrink: 0, border: `2px solid ${selected ? "#22c55e" : "rgba(255,255,255,.3)"}`, background: selected ? "#22c55e" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {selected && <Check style={{ width: 9, height: 9, color: "#fff" }} />}
      </div>
      <span style={{ fontSize: 13, color: selected ? "#fff" : "rgba(255,255,255,.7)", textAlign: "left", flex: 1 }}>{device.label || `${device.kind} — ${device.deviceId.slice(0, 8)}`}</span>
    </button>
  );

  const SectionLabel = ({ children }: { children: React.ReactNode }) => (
    <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, color: "rgba(255,255,255,.35)", letterSpacing: 1, marginBottom: 8, marginTop: 18 }}>{children}</div>
  );

  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,.65)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "#17202a", borderRadius: 20, width: "min(460px,96vw)", maxHeight: "85vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 24px 64px rgba(0,0,0,.7)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", padding: "18px 20px", borderBottom: "1px solid rgba(255,255,255,.07)" }}>
          ...
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,.1)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X style={{ width: 15, height: 15 }} /></button>
        </div>
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,.07)" }}>
          {(["audio", "video"] as const).map(tb => (
            <button key={tb} onClick={() => setTab(tb)} style={{ flex: 1, padding: "12px 6px", background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: tab === tb ? 700 : 400, color: tab === tb ? "#fff" : "rgba(255,255,255,.4)", borderBottom: `2px solid ${tab === tb ? TEAL : "transparent"}`, transition: ".15s" }}>{tb === "audio" ? "🎙️ Audio" : "📹 Video"}</button>
          ))}
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 20px 20px" }}>
          {tab === "audio" && (<>
            <SectionLabel>Microphone (incl. Bluetooth)</SectionLabel>
            {audioIn.length === 0 ? <p style={{ fontSize: 13, color: "rgba(255,255,255,.35)" }}>No microphones found</p> : audioIn.map(d => <DeviceRow key={d.deviceId} device={d} selected={selAudioIn === d.deviceId} onClick={() => switchDevice("audioinput", d.deviceId)} />)}
            <SectionLabel>Speaker / Headset / Bluetooth</SectionLabel>
            {audioOut.length === 0 ? <p style={{ fontSize: 13, color: "rgba(255,255,255,.35)" }}>Output switching not supported on this browser</p> : audioOut.map(d => <DeviceRow key={d.deviceId} device={d} selected={selAudioOut === d.deviceId} onClick={() => switchDevice("audiooutput", d.deviceId)} />)}
          </>)}
          {tab === "video" && (<>
            <SectionLabel>Camera (incl. Front / Back)</SectionLabel>
            {videoIn.length === 0 ? <p style={{ fontSize: 13, color: "rgba(255,255,255,.35)" }}>No cameras found</p> : videoIn.map(d => <DeviceRow key={d.deviceId} device={d} selected={selVideoIn === d.deviceId} onClick={() => switchDevice("videoinput", d.deviceId)} />)}
...
const ParticipantTile = ({ participant, isLocal, size = "normal" }: { participant: any; isLocal: boolean; size?: "normal" | "large" | "small" }) => {
  ...
  const attachVideo = useCallback(() => {
    ...
    if (mst && mst.readyState === "live" && !pub?.isMuted && videoRef.current) {
      if (!(videoRef.current.srcObject instanceof MediaStream) || !(videoRef.current.srcObject as MediaStream).getTracks().includes(mst)) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", background: "linear-gradient(145deg,#1a2035,#0e1420)", borderRadius: size === "small" ? 10 : 14, overflow: "hidden", border: isSpeaking ? `2px solid ${GREEN}` : "2px solid rgba(255,255,255,.06)", transition: "border-color .2s,box-shadow .2s", boxShadow: isSpeaking ? `0 0 0 2px ${GREEN}44,0 4px 24px rgba(34,197,94,.25)` : "0 2px 12px rgba(0,0,0,.4)" }}>
      ...
      {!hasVideo && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(145deg,#0e1a14,#1a2e22)" }}><div style={{ width: avatarSz, height: avatarSz, borderRadius: "50%", background: `linear-gradient(135deg,${TEAL},#064E3B)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize, fontWeight: 800, color: "#fff", border: isSpeaking ? `3px solid ${GREEN}` : "3px solid rgba(255,255,255,.1)", boxShadow: isSpeaking ? `0 0 16px ${GREEN}55` : "none", transition: "border-color .2s,box-shadow .2s" }}>{initials}</div></div>}
...
/* ══ VIDEO GRID — Google Meet style ══ */const VideoGrid = ({ layout = "grid" }: { layout?: LayoutMode }) => {
  ...
  if (screensharer) return (
    <div style={{ width: "100%", height: "100%", display: "flex", gap: GAP, padding: P, boxSizing: "border-box" }}>
      ...
      <div style={{ width: 110, display: "flex", flexDirection: "column", gap: GAP, overflowY: "auto" }}>{all.map(p => (<div key={p.identity} style={{ height: 82, flexShrink: 0 }}><ParticipantTile participant={p} isLocal={p.identity === localParticipant?.identity} size="small" /></div>))}</div>
    </div>
  );
  ...
  if (layout === "spotlight") return (
    <div style={{ width: "100%", height: "100%", display: "flex", gap: GAP, padding: P, boxSizing: "border-box" }}>
      ...
      {n > 1 && <div style={{ width: 100, display: "flex", flexDirection: "column", gap: GAP, overflowY: "auto" }}>{all.slice(1).map(p => (<div key={p.identity} style={{ height: 76, flexShrink: 0 }}><ParticipantTile participant={p} isLocal={p.identity === localParticipant?.identity} size="small" /></div>))}</div>}
    </div>
  );
  ...
  if (layout === "focus") {
    ...
    return (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", gap: GAP, padding: P, boxSizing: "border-box" }}>
        ...
        {others.length > 0 && <div style={{ height: 90, display: "flex", flexDirection: "row", gap: GAP, flexShrink: 0, overflowX: "auto" }}>{others.map(p => (<div key={p.identity} style={{ width: 120, flexShrink: 0 }}><ParticipantTile participant={p} isLocal={false} size="small" /></div>))}</div>}
      </div>
    );
  }
  if (n === 1) return (
    ...
    <div style={{ position: "fixed", bottom: isMobile ? 66 : 82, left: isMobile ? 4 : 16, background: "#1a2232", border: "1px solid rgba(255,255,255,.12)", borderRadius: 16, minWidth: 260, zIndex: 9100, boxShadow: "0 12px 40px rgba(0,0,0,.7)", overflow: "hidden", animation: "slide-up .15s ease" }}>
      ...
      <div style={{ padding: "6px 0", maxHeight: 220, overflowY: "auto" }}>
        {audioDevices.filter(d => d.kind === "audioinput").map(d => (
          <button key={d.deviceId} onClick={() => { switchAudioDevice(d.deviceId, "audioinput"); setAudioPickerOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
    ...
    <div style={{ position: "fixed", bottom: isMobile ? 66 : 82, left: isMobile ? 56 : 76, background: "#1a2232", border: "1px solid rgba(255,255,255,.12)", borderRadius: 16, minWidth: 260, zIndex: 9100, boxShadow: "0 12px 40px rgba(0,0,0,.7)", overflow: "hidden", animation: "slide-up .15s ease" }}>
      ...
      <div style={{ padding: "6px 0", maxHeight: 220, overflowY: "auto" }}>
        ...
        {videoDevices.length > 1 && (
          <button onClick={() => { flipCamera(); setVideoPickerOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "rgba(255,255,255,.04)", border: "none", cursor: "pointer", textAlign: "left", borderTop: "1px solid rgba(255,255,255,.06)" }}>
    ...
    {moreOpen && menuPortal && createPortal(
      <div style={{ position: "fixed", bottom: isMobile ? 66 : 82, right: isMobile ? 60 : 76, background: "#17202a", border: "1px solid rgba(255,255,255,.08)", borderRadius: 18, boxShadow: "0 8px 36px rgba(0,0,0,.65)", minWidth: 240, zIndex: 9100, overflow: "hidden", animation: "slide-up .15s ease" }}>
        <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,.35)", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>View</div>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {([["horizontal", "🟦", "Side by side"], ["grid", "⋂", "Grid"], ["spotlight", "⛶", "Spotlight"], ["focus", "▢", "Focus"]] as const).map(([m, ic, lb]) => (
              <button key={m} onClick={() => { onLayoutChange(m); }} style={{ padding: "5px 10px", borderRadius: 8, border: "1px solid", fontSize: 11, fontWeight: 600, cursor: "pointer", borderColor: layout === m ? "#22c55e" : "rgba(255,255,255,.12)", background: layout === m ? "rgba(34,197,94,.14)" : "rgba(255,255,255,.04)", color: layout === m ? "#22c55e" : "rgba(255,255,255,.55)" }}>{lb}</button>
            ))}
          </div>
        </div>
        {[
        ...
        ].map((item, i) => (<button key={"p" + i} onClick={item.fn} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "none", border: "none", cursor: "pointer", color: item.color, fontSize: 13, borderBottom: "1px solid rgba(255,255,255,.05)", textAlign: "left" }}><item.icon style={{ width: 15, height: 15 }} />{item.label}</button>))}
        {!isPrivileged && canStudentRecProp && (
          <button onClick={() => { toggleStuRecord(); setMoreOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "none", border: "none", cursor: "pointer", color: stuRec ? "#ef4444" : "#fff", fontSize: 13, borderBottom: "1px solid rgba(255,255,255,.05)", textAlign: "left" }}>
            <Circle style={{ width: 12, height: 12, fill: stuRec ? "#ef4444" : "none", color: stuRec ? "#ef4444" : "#fff" }} />{stuRec ? "Stop Recording" : "Record Audio"}
          </button>
        )}
        {onMinimize && <button onClick={() => { onMinimize(); setMoreOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "none", border: "none", cursor: "pointer", color: "#fff", fontSize: 13, borderBottom: "1px solid rgba(255,255,255,.05)", textAlign: "left" }}><ChevronDown style={{ width: 15, height: 15 }} /> Minimize</button>}
      </div>, menuPortal)}
...
const ClassroomView = ({ subject, onLeave, onMinimize, autoJoin = false }: ClassroomViewProps) => {
  ...
  const endSession = async () => {
    ...
    } catch (e: any) { console.error("[endSession] DB error (continuing anyway): ", e?.message); } finally { setPhase("ended"); }
  };
