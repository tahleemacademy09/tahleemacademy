...
const InlinePlayer = ({ recordingId, fileUrl, duration, onClose }: {
  ...
  useEffect(() => {
    ...
    // Use the centralised helper — it picks the right bucket automatically
    // and handles legacy full-URL entries gracefully.
    getSignedUrl(fileUrl, 7200).then(url => {
      ...
      if (url) {
        ...
        setError("Could not load recording. The file may have been deleted or the storage bucket is not configured.");
      }
      setLoading(false);
    }).catch(err => {
      if (!cancelled) {
        setError("Failed to generate playback URL: " + (err?.message || "Unknown error"));
        setLoading(false);
      }
    });
    ...
    <div style={{ background: "#0a0a0a", borderRadius: 16, overflow: "hidden", marginBottom: 16 }}>
      {loading ? (
        ...
        <div style={{ padding: 20, textAlign: "center", color: "#EF4444", fontSize: 13 }}>
          ...
          <button onClick={onClose} style={{ background: "rgba(255,255,255,.1)", border: "none", color: "#fff", cursor: "pointer", padding: "4px 12px", borderRadius: 8, fontSize: 12 }}>
            Close
          </button>
        </div>
      ) : signedUrl ? (
...
const SubjectRecordings = ({ subjectId }: { subjectId: string }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, teacher_name, duration_seconds }: any) => {
      const { error } = await supabase
        .from("session_recordings")
        .update({ teacher_name, duration_seconds })
        .eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      // Remove the actual file from storage — picks correct bucket automatically
      if (rec?.file_url) {
        await removeStorageFile(rec.file_url).catch(err =>
          console.warn("[SubjectRecordings] storage remove failed (continuing):", err?.message)
        );
      }
