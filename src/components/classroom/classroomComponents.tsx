...
// Persisted per-browser (not per-account — this is about a specific
// camera/device, which doesn't change based on who's logged in on it), OFF
// by default. See the perf note on CameraUnmirrorEngine below for why this
// isn't just always on.
const MIRROR_FIX_KEY = "ta_camera_unmirror_fix";
export const getMirrorFixEnabled = () => {
  try { return localStorage.getItem(MIRROR_FIX_KEY) === "1"; } catch { return false; }
};
export const setMirrorFixEnabled = (on: boolean) => {
  try { localStorage.setItem(MIRROR_FIX_KEY, on ? "1" : "0"); } catch {}
  try { window.dispatchEvent(new Event("ta-mirror-fix-changed")); } catch {}
};
...
//
// BUG FIX ("video is very slow, not prompt"): this processor works by
// decoding the camera into an offscreen <video>, redrawing every single
// frame onto a canvas, and re-encoding THAT as the published stream — a
// real CPU and latency cost, on top of the normal capture→encode pipeline,
// paid on every frame for as long as the camera is on. Mirrored-source
// cameras are an uncommon driver quirk, not something every device has —
// but this was mounted unconditionally, so EVERY participant's device,
// including students who never had a mirroring problem in the first
// place, was paying that processing cost for a fix they didn't need. That
// extra per-frame overhead is what was making video feel laggy/delayed
// class-wide. Now gated behind an explicit, OFF-by-default opt-in (Room
// Settings → Video → "My Camera Looks Backwards To Others") — only the
// specific device that actually needs correction pays for it, and
// everyone else's camera publishes at full native speed with zero
// processing overhead, same as before any of this was added.
export const CameraUnmirrorEngine = () => {
  const room = useRoomContext();
  const processedTrackId = useRef<string | null>(null);
  const [enabled, setEnabled] = useState(getMirrorFixEnabled());

  useEffect(() => {
    const onChange = () => setEnabled(getMirrorFixEnabled());
    window.addEventListener("ta-mirror-fix-changed", onChange);
    window.addEventListener("storage", onChange);
    return () => {
      window.removeEventListener("ta-mirror-fix-changed", onChange);
...
export const MaterialPicker=({subjectId,onShare,onClose}:any)=>{
  ...
  // Sharing here broadcasts live to the whole class, so only surface
  // admin-authorized materials — same rule as the follow-the-teacher sync.
  useEffect(()=>{supabase.from("subject_materials").select("*").eq("subject_id",subjectId).eq("sync_allowed" as any,true).order("created_at",{ascending:false}).then(({data})=>{setMats(data||[]);setBusy(false);});},[subjectId]);
  return createPortal(<div style={{position:"fixed",inset:0,zIndex:9997,background:"rgba(0,0,0,.72)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-end"}} onClick={onClose}>
    <div style={{width:"100%",background:"#17202a",borderRadius:"22px 22px 0 0",maxHeight:"70vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"6px 0"}}>
        ...
        {!busy&&!mats.length&&<p style={{textAlign:"center",padding:"28px",color:"rgba(255,255,255,.35)",fontSize:14}}>No sync-authorized materials yet — ask an admin to authorize one from the Materials panel.</p>}
...
export const InClassMaterialViewer=({material,onClose,isTeacher=false,onMinimize,fromPanel=false,syncActive=false,onSyncActivity,canSync=false,syncOn=false,onToggleSync,following=false,remoteActivity=null}:any)=>{
  ...
  const renderContent=()=>{
    ...
    // Scroll position follows the same teacher→student sync channel used for
    // HTML lessons (syncActive/onSyncActivity on the teacher's copy,
    // following/remoteActivity on the student's copy).
    if(kind==="pdf")return(
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0,position:"relative"}}>
        <PDFViewer url={embedUrl} bg="#0f1117" materialId={matId}
          syncActive={isTeacher&&syncActive}
          onSyncScroll={(pct:number)=>onSyncActivity?.(0.5,0.5,pct,null,null,null)}
          following={following}
          remoteScrollPct={following&&remoteActivity?remoteActivity.scrollPct:null}
        />
        {/* While following, the student's copy is fully teacher-driven — block
            local interaction so their own scroll/zoom/page taps can't fight
            the incoming sync, same as the HTML lesson follow-lock. */}
        {following&&<div style={{position:"absolute",inset:0,zIndex:5}}/>}
      </div>
    );
    if(kind==="html")return(
...
export const SubjectMaterialsPanel=({subjectId,subject,sessionId,onClose,canStudentRec,isPrivileged,isAdmin,stuRec,onToggleStuRecord,onOpenMatsChange,panelRef}:any)=>{
  const{user}=useAuth();
  ...
  const[cSyncAllowed,setCSyncAllowed]=useState(false); // admin-only: authorize this material for live sync at upload time
  ...
  // ── Admin-only: authorize/revoke live sync for a material ──────────────
  ...
  const resetComposer=()=>{
    setCTitle("");setCBody("");setCFile(null);setCError("");setCVisibility("all");setCSyncAllowed(false);
              ...
              <div style={{padding:"0 14px 14px"}}>
                ...
                {isAdmin&&(
                  <button onClick={()=>setCSyncAllowed(v=>!v)} style={{
                    ...
                    background:cSyncAllowed?"rgba(52,211,153,.1)":"transparent",
                  }}>
                    <span style={{fontSize:14}}>{cSyncAllowed?"🔗":"🔒"}</span>
                    <span style={{fontSize:11,fontWeight:600,color:cSyncAllowed?"#34d399":"rgba(255,255,255,.5)",textAlign:"left" as const,flex:1}}>
                      Allow live sync to students{cSyncAllowed?" — authorized":""}
                    </span>
                  </button>
                )}
                {/* Mode toggle */}
...
export const RoomSettingsModal = ({ onClose, room }: { onClose: () => void; room: any }) => {
  ...
  const [mirrorFixOn, setMirrorFixOn] = useState(getMirrorFixEnabled());
  const setMirrorFix = (on: boolean) => { setMirrorFixOn(on); setMirrorFixEnabled(on); };
        ...
        <div style={{ flex:1,overflowY:"auto",padding:"4px 20px 20px" }}>
          ...
          {tab==="video"&&(
            <>
              ...
              <SectionLabel>My Camera Looks Backwards To Others</SectionLabel>
              {/* Opt-in, OFF by default — see CameraUnmirrorEngine for why:
                  correcting a mirrored camera means running every frame
                  through a canvas before it's published, which has a real
                  CPU/latency cost. Only the small number of people whose
                  actual camera hardware pre-mirrors its feed should pay
                  that cost — flipping this on for everyone by default is
                  what caused the classroom-wide video slowness. Takes
                  effect the next time the camera is turned on (leave the
                  call and rejoin, or toggle the camera off/on, to apply it
                  to an already-running camera). */}
              <label style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.03)",cursor:"pointer" }}>
                <input type="checkbox" checked={mirrorFixOn} onChange={(e)=>setMirrorFix(e.target.checked)} style={{width:16,height:16,flexShrink:0}}/>
                <span style={{fontSize:13,color:"rgba(255,255,255,.75)"}}>
                  Fix it (others see your video mirror-flipped / text backwards)
                </span>
              </label>
              <p style={{ fontSize:11,color:"rgba(255,255,255,.3)",marginTop:6,lineHeight:1.6 }}>
                Only turn this on if people tell you YOUR video looks backwards to them — it adds some processing overhead, so leave it off otherwise.
              </p>
            </>
          )}
        </div>
...
export const ParticipantTile=({participant,isLocal,size="normal",pip=false}:{participant:any;isLocal:boolean;size?:"normal"|"large"|"small";pip?:boolean})=>{
  ...
  const removeFromTile=async()=>{
    // BUG FIX: this used to silently do nothing if adminSessionId wasn't
    // populated yet (a real gap right after joining — ClassroomView fills
    // sessionId in via background bookkeeping a moment after connecting).
    // Clicking Remove in that window looked like it did nothing at all.
    if(!adminSessionId){
      toast({title:"Still setting up the class session — try again in a moment",variant:"destructive"});
      return;
    }
    ...
    try{
      ...
      if(error||data?.error)throw new Error(data?.error||error?.message);
      // BUG FIX ("Remove from class didn't actually end their call"): the
      // DB half of this can succeed even when the actual LiveKit disconnect
      // failed server-side — moderate-participant now reports that
      // separately as liveKitError instead of silently swallowing it, so
      // surface it here rather than showing a false "removed" toast.
      if(data?.liveKitError){
        toast({title:`${name} was removed from the class list, but the live disconnect failed`,description:data.liveKitError,variant:"destructive"});
      }else{
  ...
  const blockFromTile=async()=>{
    ...
    try{
      ...
      if(error||data?.error)throw new Error(data?.error||error?.message);
      if(data?.liveKitError){
        toast({title:`${name} was blocked, but the live disconnect failed`,description:data.liveKitError,variant:"destructive"});
      }else{
    ...
    >
      ...
      {!pip&&(
        <div style={{
          ...
          {canModerate ? (
            <button data-tile-control="true" onClick={toggleTileMic} onPointerDown={(e:any)=>e.stopPropagation()} title={micEnabled?"Mute":"Unmute"}
      ...
      {canModerate&&tileMenuOpen&&(
        <div data-tile-control="true" onClick={(e:any)=>{e.stopPropagation();setTileMenuOpen(false);}} style={{
...
export const DuoPipLayout=({participants,localIdentity}:{participants:any[];localIdentity?:string})=>{
  ...
  // BUG FIX ("click mic to mute — it just swaps the view instead", only in
  // the 2-person DuoPip layout): these two wrapping divs are what make the
  // WHOLE tile tappable-to-swap, including the area the tile's own mic
  // button and long-press menu occupy. The controls already call
  // e.stopPropagation() on click, but that only protects against OTHER
  // React onClick handlers — it does nothing against a raw failed-tap
  // landing between the button's actual clickable box and its visual
  // bounds on a small mobile hit target, and any ambiguity there was
  // resolving in favour of "swap" instead of the tile control underneath.
  // Explicitly ignoring clicks whose target is (or is inside) a
  // [data-tile-control] element removes that ambiguity outright, so the
  // mic button and the remove/block menu are always reliably clickable
  // even in this layout, regardless of hit-target overlap.
  const swap=(e:any)=>{
    if(e?.target?.closest?.("[data-tile-control]"))return;
    setOwnerIsPip(v=>!v);
  };
  ...
  return(
    <div style={{width:"100%",height:"100%",position:"relative",background:"#0a0a0a",overflow:"hidden"}}>
      ...
      <div
        onClick={(e:any)=>{e.stopPropagation();swap(e);}}
        style={{
