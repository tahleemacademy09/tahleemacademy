...
const MINIMIZE_GRACE_MS = 30 * 60 * 1000; // 30 minutes — matches WhatsApp/Meet behaviour

const ReconnectMonitor = ({ onReconnecting, onReconnected, onDisconnected, onDisconnectedSilent }: {
  onReconnecting: () => void;
  onReconnected:  () => void;
  onDisconnected: () => void;
  onDisconnectedSilent: () => void;
}) => {
    ...
    const onVis = async () => {
      if (document.visibilityState === "hidden") {
        ...
        graceTimer.current = setTimeout(() => {
          ...
          // Grace elapsed — silent reconnect if WS dropped
          if (wsDropped.current || room.state === ConnectionState.Disconnected) {
            onDisconnectedSilent();
          }
        }, MINIMIZE_GRACE_MS);
        return;
      }
      ...
      if (room.state === ConnectionState.Disconnected) {
        // Returning from background → silent reconnect (no overlay flash)
        if (wasHidden) { onDisconnectedSilent(); } else { onDisconnected(); }
        return;
      }
      try {
  ...
  }, [room, onReconnecting, onReconnected, onDisconnected, onDisconnectedSilent]);
  return null;
};
...
const SubjectMaterialsPanel=({subjectId,subject,sessionId,onClose,canStudentRec,isPrivileged,stuRec,onToggleStuRecord,onOpenMatsChange,panelRef}:any)=>{
  ...
  const minimizeActive=()=>{setActiveMatId(null);setListVisible(false);setForceList(false);};
...
const ClassroomView=({subject,onLeave,onMinimize,autoJoin=false}:ClassroomViewProps)=>{
  ...
  const autoReconnect=useCallback(async(silent=false)=>{
    ...
    // silent=true when returning from background — don't flash the reconnecting overlay
      ...
      {token&&wsUrl&&(
        ...
        <LiveKitRoom key={roomKey} serverUrl={wsUrl} token={token} connect={phase==="live"} audio={false} video={false} options={{adaptiveStream:{pixelDensity:"screen"},dynacast:true,disconnectOnPageLeave:false,audioCaptureDefaults:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,sampleRate:48000,channelCount:1},publishDefaults:{audioPreset:{maxBitrate:64000},dtx:false,red:true,stopMicTrackOnMute:false,videoEncoding:{maxBitrate:700_000,maxFramerate:20},backupCodec:true},videoCaptureDefaults:{resolution:{width:640,height:480,frameRate:20},facingMode:"user"}}} style={{flex:1,display:"flex",flexDirection:"column",minHeight:0,position:"relative"}} data-lk-theme="default">
          ...
          <ReconnectMonitor
            ...
            onDisconnectedSilent={()=>autoReconnect(true)}
          />
